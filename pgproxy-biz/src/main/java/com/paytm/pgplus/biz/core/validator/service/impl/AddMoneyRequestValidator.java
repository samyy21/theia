...
@Service("addMoneyValidator")
public class AddMoneyRequestValidator implements IValidator {
    ...
    public GenericCoreResponseBean<Boolean> validate(final WorkFlowRequestBean workFlowBean) {
        ...
        if ((errorMessage == null)
                && (!BizParamValidator.validateInputObjectParam(workFlowBean.getRequestType()) || !(workFlowBean
                        .getRequestType().equals(ERequestType.ADD_MONEY)
                        || workFlowBean.getRequestType().equals(ERequestType.TOPUP_EXPRESS) || workFlowBean
                            .isNativeAddMoney()))) {
        ...
        if ((errorMessage == null) && !BizParamValidator.validateInputObjectParam(workFlowBean.getFundType())) {
            LOGGER.info("validation failed as invalid FundTypeID is Passed");
            errorMessage = "InvalidFundTypeID " + workFlowBean.getFundType();
            responseConstant = ResponseConstants.INVALID_PARAM;
        }
