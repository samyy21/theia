...
@Service("filterTokenizedAndPlatformSavedCardsTask")
public class FilterTokenizedAndPlatformSavedCardsTask extends
    ...
    private static final ExtendedLogger EXT_LOGGER = ExtendedLogger
            .create(FilterTokenizedAndPlatformSavedCardsTask.class);
    private static final Logger LOGGER = LoggerFactory.getLogger(FilterTokenizedAndPlatformSavedCardsTask.class);
    ...
    protected GenericCoreResponseBean<LitePayviewConsultResponseBizBean> doBizExecute(WorkFlowRequestBean input,
        ...
        if (tokenizedCardsResponseBean != null && !CollectionUtils.isEmpty(tokenizedCardsResponseBean.getCardInfos())) {
            ...
            List<String> disabledPaymentModes = transBean.getWorkFlowBean().getDisabledPaymentModes();
            Iterator itr = tokenizedCards.iterator();
            while (itr.hasNext()) {
                CardInfo card = (CardInfo) itr.next();
                if (!BizConstant.STATUS_ACTIVE.equals(card.getTokenStatus())
                        || StringUtils.isEmpty(card.getGlobalPanIndex())
                        || (!CollectionUtils.isEmpty(disabledPaymentModes) && disabledPaymentModes
                                .contains(BizConstant.CC.equals(card.getCardType()) ? CREDIT_CARD : DEBIT_CARD))) {
                    if (StringUtils.isEmpty(card.getGlobalPanIndex())) {
        ...
        LOGGER.info("Unique Tokenized cards after dedup logic with LPV cards : {}", uniqueTokenCards);
                ...
                if (payMethodView != null
                    ...
                    while (paymethodCardOptionIterator.hasNext()) {
                        ...
                        if (!coftUtil.checkTokenProcessingEnable(issuerTokenProcessingOnMid, txnAmt, issuer)
                                || !cardTransactionUtil.isIssuerConfigAllowedOnMid(transBean.getWorkFlowBean()
                                        .getRequestType(), transBean.getWorkFlowBean().getPaytmMID(), issuer, false)) {
    ...
    public TaskName getTaskName() {
        return TaskName.FILTER_TOKENIZED_AND_PLATFORM_CARDS;
    }
