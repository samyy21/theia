 ...
 *
 */
@Service("paytmExpessRequestValidator")
public class PaytmExpressRequestValidator implements IValidator {

    public static final Logger LOGGER = LoggerFactory.getLogger(PaytmExpressRequestValidator.class);
    ...
    public GenericCoreResponseBean<Boolean> validate(WorkFlowRequestBean workFlowBean) {
        ...
        if ((errorMessage == null) && !BizParamValidator.validateInputStringParam(workFlowBean.getWebsite())) {
            LOGGER.info("validation failed as invalid website is Passed");
            errorMessage = "InvalidWebsite " + workFlowBean.getToken();
            responseConstant = ResponseConstants.INVALID_PARAM;
        }
