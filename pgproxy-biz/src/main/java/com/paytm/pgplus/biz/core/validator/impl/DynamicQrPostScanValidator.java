...
@Service("dynamicQrPostScanValidator")
public class DynamicQrPostScanValidator implements IValidator, ICardValidator {

    public static final Logger LOGGER = LoggerFactory.getLogger(DynamicQrPostScanValidator.class);
    ...
    public GenericCoreResponseBean<Boolean> validate(final WorkFlowRequestBean workFlowBean) {
        ...
        if (StringUtils.isBlank(errorMessage)
                && !(PaymentTypeIdEnum.CC.getValue().equals(paymentTypeId)
                        ...
                        || PaymentTypeIdEnum.PPI.getValue().equals(paymentTypeId)
                        || PaymentTypeIdEnum.PAYTM_DIGITAL_CREDIT.getValue().equals(paymentTypeId)
                        || PaymentTypeIdEnum.EMI.getValue().equals(paymentTypeId) || PaymentTypeIdEnum.GIFT_VOUCHER.value
        ...
        if (StringUtils.isBlank(errorMessage) && !BizParamValidator.validateTipAmount(workFlowBean)) {
            LOGGER.error("validation failed as invalid TipAmount is Passed");
            errorMessage = "InvalidTipAmount";
            responseConstant = ResponseConstants.INVALID_TIP_AMOUNT;
        }
