...
@Service("invoicePaymentPreValidator")
public class InvoicePaymentPreRequestValidator implements IValidator {
    public static final Logger LOGGER = LoggerFactory.getLogger(InvoicePaymentPreRequestValidator.class);
    ...
    public GenericCoreResponseBean<Boolean> validate(final WorkFlowRequestBean workFlowBean) {
         ...
         * !BizParamValidator.validateChannelID(workFlowBean.getChannelID())) {
         * LOGGER.info( "validation failed as invalid ChannelID is Passed");
         * errorMessage = "InvalidChannelID " + workFlowBean.getChannelID(); }
         */

        if ((errorMessage == null)
                && (!BizParamValidator.validateInputStringParam(workFlowBean.getCallBackURL()) && !BizParamValidator
                        .validateInputStringParam(workFlowBean.getWebsite()))) {
            LOGGER.info("validation failed as CallBackURL & Website are passed blank");
            errorMessage = "CallBackURL & Website are passed blank";
            responseConstant = ResponseConstants.INVALID_PARAM;
        }
