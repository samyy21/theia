...
@Service("normalPostLoginValidator")
public class NormalPostLoginRequestValidator implements IValidator {
    public static final Logger LOGGER = LoggerFactory.getLogger(NormalPostLoginRequestValidator.class);
    ...
    public GenericCoreResponseBean<Boolean> validate(final WorkFlowRequestBean workFlowBean) {
        ...
        if ((errorMessage == null) && !BizParamValidator.validateInputStringParam(workFlowBean.getoAuthCode())) {
            LOGGER.info("validation failed as invalid OAuthCode is Passed");
            errorMessage = "InvalidAuthCode";
            responseConstant = ResponseConstants.INVALID_PARAM;
        }
