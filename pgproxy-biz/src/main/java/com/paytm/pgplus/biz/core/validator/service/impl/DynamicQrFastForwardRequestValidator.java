...
@Service("dynamicQrFastForwardRequestValidator")
public class DynamicQrFastForwardRequestValidator implements IValidator {

    public static final Logger LOGGER = LoggerFactory.getLogger(DynamicQrFastForwardRequestValidator.class);
    ...
    public GenericCoreResponseBean<Boolean> validate(WorkFlowRequestBean workFlowBean) {
        ...
        if ((errorMessage == null) && !BizParamValidator.validateTipAmount(workFlowBean)) {
            LOGGER.info("validation failed as invalid TipAmount is Passed");
            errorMessage = "InvalidTipAmount " + workFlowBean.getTipAmount();
