...
<c:if test="${2 eq paymentType || 1 eq paymentType}">
	<c:choose>
		...
		<c:otherwise>
			<div class="heading" style="margin-top:10;">Credit / Debit Card</div>
			...
			<form autocomplete="off" method="post" action="${pageContext.request.contextPath}/payment/request/submit?${queryStringForSession}" onsubmit = "return submitForm(this)" id="DC-form"
				...
				<div class="row-dot" style="padding:0;">

					<div style="box-sizing: border-box;
    margin: 3px 10px;
    background: #efefef;
    border-radius: 5px; padding:7px 10px;">
						<p class="error" style="border-bottom: 1px solid #ccc;padding-bottom: 7px;">
							...
							<input id="card-number" placeholder="Card Number" type="tel" maxlength="23" name="cardNumber" class="mt5 ${cssClass}" value="${!empty cardNumber ? cardNumber : defaultCardNumber}" onblur="onCardNumberBlur('dc')" onkeydown="pressCardSpace(event);" onkeyup="onCardDown(false,event); removeFieldError(null, this)" style="width:100%; box-sizing:border-box; font-size: 12px;"/>
						...
						<div id="cvv_expiry_block" style="position:relative; overflow:hidden;padding-top: 0px;">
        <span style="border-right: 1px solid #ccc;

    padding-right: 32px; position:absolute; height:100%; left: 46%;
    top: 5px;"></span>
							<p class="pt7">
							<div class="fl">
								...
								<select name="expiryMonth" id="dcMonth" class="expSelect mt5 ${cssClass}" onchange="removeFieldError(null, this)" style="color:#7f7575; font-size: 12px;">
							...
							<div class="fl ml20">
								...
								<select name="expiryYear" id="dcYear" class="expSelect width mt5 ${cssClass}" onchange="removeFieldError(null, this)" style="color:#7f7575; font-size: 12px;">
							...
							<p class="pt7 cvv" id ="normalOptionCVV" style="
    float: right;
    margin-top: -7px;">
					...
					<div id="idebitPayOption" style="margin-left: 6px;" class="mt20 hide">
						...
						<div class = "clear"></div>
						<p class="pt7 cvv hide" style="margin-left:20px;" id ="idebitOptionCVV">
							<label for="cvvNumber">CVV</label><br />
							...
							<input type="tel" id="idebitOptionCVVTxt" placeholder="***" maxlength="4" size="5" name="cvvNumber" class ="mask cvv-input ${cssClass}" onkeyup="onKeypressCvvNumber(arguments[0]); removeFieldError(null, this)"/><img id="defaultCvvIcon" src="${ptm:stResPath()}images/wap/paytmAppLow/cvv.png" alt="CVV" title="CVV" /><img id="amexCvvIcon" class="hide" src="${ptm:stResPath()}images/wap/paytmAppLow/security-code.png" alt="Security Code" title="Amex Cvv" />
								${errorText}
						</p>
					</div>
						<%-- --         </c:if> -- --%>
						...
						<c:choose>
							<c:when test="${themeInfo.subTheme eq 'ccdc' }">
								<p class="pt7">
									...
									<input type="checkbox" id="saveCard" name="storeCardFlag" value="Y"
										   checked="checked" disabled="disabled"/> Save this card for future checkout
								</p>
							</c:when>
								...
								<p class="pt7" id="dcCardSaved">
									<input type="checkbox" id="saveCard" name="storeCardFlag" value="Y"
										${txnConfig.saveCardMandatory ? 'disabled="disabled"':''}
										   checked="checked"/> Save this card for faster checkout
					...
					<div id="warningDiv" class = "clear failure" style="display: none; width: 90%; margin-left: 10px;"><span id="errorMsg"></span></div>
					<p class="small mb10 hide pt7" id="maestro_msg" style="padding-left: 15px;">
					...
					<p class="pt7 row-dot">
						<button type="submit" id="btnSubmit" class="blue-btn dc-blue-btn">Pay Now</button>
					</p>
				</div>
			</form>
