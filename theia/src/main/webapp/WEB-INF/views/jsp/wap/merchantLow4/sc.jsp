...
<fmt:parseNumber var="minAmountForAddMoneyVoucher" pattern="0.0" type="number" value="${txnConfig.minAmountForAddMoneyVoucher}" />
		...
		<c:forEach var="card" varStatus="status" items="${savedCardList}">
			...
			<div class="row-dot sc-row ${cardState}" onclick="onSaveCardTabClick(event,this)" data-firstsixdigits="${card.firstSixDigit}" data-txnMode="${card.txnMode}">
...
<%-- 				</c:if> --%>
				...
				<c:if test="${!card.iDebitCard}">
					<p class="pad10 cvv-box">
						...
						<input type="tel" name="cvvBox" maxlength="${isIMPS ? '8' : '4'}" size="9" id = "cvv_${card.cardId}_input" autocomplete="off" class ="mask cvv-input ${isIMPS ? 'medium-input' : ''} ${cssClass}" onkeyup="onKeypressCvvNumber(arguments[0])"/>
					...
					<c:if test="${card.iDebitCard}">
						<div class="pt7 ml20">
							...
							<p class="pad10 cvv-box hide">
								...
								<input type="tel" maxlength="${isIMPS ? '8' : '4'}" size="9" id = "cvv_${card.cardId}_input" autocomplete="off" class ="mask cvv-input ${isIMPS ? 'medium-input' : ''} ${cssClass}" onkeyup="onKeypressCvvNumber(arguments[0])"/>
								<img src="${ptm:stResPath()}images/wap/merchantLow/cvv.png" alt="CVV" title="CVV" />
									${errorText}
							</p>
							<!-- For Icici Debit card Only -->
					...
					<c:if test="${card.cardType eq 'CREDIT_CARD' && txnConfig != null &&  txnConfig.paymentCharges != null && txnConfig.paymentCharges.CC != null}">
					...
					<c:if test="${card.cardType eq 'DEBIT_CARD' && txnConfig != null &&  txnConfig.paymentCharges != null && txnConfig.paymentCharges.DC != null}">
					...
					<p class="pt7 ml20">
					 	<button type="submit" class="blue-btn" id="btnSubmit-${card.cardId}">Pay Now</button>
					</p>
					<div class="pt7 ml20">
			...
			<c:if test="${card.paymentMode eq 'UPI'}">
				...
				<div class="row-dot sc-row ${cardState}" onclick="onSaveCardTabClick(event,this)" data-firstsixdigits="${card.firstSixDigit}" data-txnMode="${card.txnMode}">
					...
					<div class="cvv" style = "display:none" id = "cvv_${card.cardId}_pad">
						...
						<c:if test="${card.cardType eq 'UPI' && txnConfig != null &&  txnConfig.paymentCharges != null && txnConfig.paymentCharges.UPI != null}">
							<div class="post-conv-inclusion" id="normal-post-con-${card.cardId}"  style="margin-left: 20px; margin-right:10px; height: 44px;float: left;">
