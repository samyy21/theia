				...
				<c:forEach var="card" varStatus="status" items="${savedCardList}">
					...
					<c:if test="${card.paymentMode eq 'UPI'}">
						<c:choose>
							...
							<c:when test="${'BAJAJFN' eq card.cardScheme}">
								<c:set var="cardImagePrefix" value="bajajfn" />
							</c:when>
							<c:when test="${'BAJAJ' eq card.cardScheme}">
								<c:set var="cardImagePrefix" value="bajaj" />
							</c:when>
						</c:choose>
						<c:set var="isIMPS" value="false"></c:set>
