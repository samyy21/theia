...
<html xmlns="http://www.w3.org/1999/xhtml" class="ui-mobile">
	...
	<c:if test="${empty header['PRX']}">
		...
		<link rel="stylesheet" type="text/css" href="css/wap/paytmHigh/mobile.css" />
		...
		<div data-role="page" class="ui-page ui-body-c ui-page-active">
			...
			<header id="header" data-showtabs="${sessionScope.promoPaymentModes}" data-promocodetype="${sessionScope.promocodeType}" data-role="header" class="ui-header ui-bar-a">
				...
				<div class="user-view fr">
		    		<c:if test ="${loginInfo.loginFlag eq 'N'}">
						<div class="myaccount fl">
							<div class="login">
								<a id="login-btn" href="#">Login / Signup</a>
							</div>
						</div>
					</c:if>
					...
					<c:if test ="${loginInfo.loginFlag eq 'Y' && !empty loginInfo.user}">
						<div class="myaccount fl">
							<div class="logout">
								<div class="fl user-name"><c:out value="${loginInfo.user.userName or loginInfo.user.emailId or loginInfo.user.mobileNumber}" escapeXml="true" /></div>
			...
			<section data-role="content" class="ui-content">
				...
				<c:if test="${walletInfo.walletBalance_INFO eq 'Y' && empty loginInfo.user}">
					<div class="failure">
						We have your wallet. Please login to avail benefits. 
					</div>
				</c:if>
					...
					<ul class="ui-grid-a">
				    	...
				    	<c:if test="${!empty walletInfo.walletBalance && walletInfo.walletBalance > 0}">
					        <li class="ui-block-a">
								...
								<div class="bal" id = "yourBal">(Your current balance is <span class="WebRupee">Rs</span> <span class = 'amt'></span>)</div>
				...
				<h1 class="mt15">
					<span class="otherText">
					 ...
					 <span id="Notbalance">Select an option to pay</span>
					  <span class = "WebRupee">Rs</span> <span class="finalAmountSpan">${txnInfo.txnAmount}</span></span>
