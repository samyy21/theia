    ...
    <div class="row mb5 rel_pos" <c:if test="${disablePaytmCashCheckbox}">style="color:#9a9999;"</c:if>>
      <div class="fl">
        <input name="paytmCashCB" id="paytmCashCB" type="checkbox" value="" onclick="processPaytmCash(this)" <c:if test="${checkPaytmCashCheckbox}">checked="checked"</c:if>  <c:if test="${disablePaytmCashCheckbox}">disabled="disabled"</c:if> />
					<span class="rel_pos span_inline" <c:if test="${disablePaytmCashCheckbox eq false}">onclick="walletItemClickable(this.previousElementSibling)"</c:if>>Paytm Wallet
...
<c:if test="${isPPBL_Enabled && themeInfo.subTheme ne 'ccdc'}">
  ...
  <c:set var="isPaymentBankCheckboxChecked" value="${!paymentBankInactive  &&
...
<c:if test="${isUPI_Push}">
    ...
    <c:if test="${isPPBL_Enabled}">
        ...
        <c:if test="${param.use_paytmcc eq 0}">
            <c:set var="isPaymentBankCheckboxChecked" value="false"></c:set>
        ...
        <form autocomplete="off"
          ...
          <div class="btn-submit fl" style="width: 100%;">
            <input class="gry-btn" type="submit"
                   ...
                   id="pbSubmit"
                    <c:if test="${themeInfo.subTheme eq 'idea' || themeInfo.subTheme eq 'ga'}"> data-txnmode="PAYMENTBANK" onclick="pushGAData(this, 'pay_now_clicked')" </c:if>
                    />
          </div>
        </form>
        <div class="clear"></div>
        ...
        <div class="btn-submit fl" style="width: 100%;">
          <c:choose>
            <c:when test ="${digitalCreditInfo.passcodeRequired}">
              <input class="gry-btn" id="postSubmit" type="submit"
              <c:if test="${themeInfo.subTheme eq 'idea' || themeInfo.subTheme eq 'ga'}"> data-txnmode="PAYTM_POSTPAID" onclick="pushGAData(this, 'pay_now_clicked')" </c:if>
                     value="Pay Now" style="width: 98%;border-radius: 5px;" name="">
            </c:when>

            <c:when test ="${!digitalCreditInfo.passcodeRequired}">
              <input class="blue-btn" id="postSubmit" type="submit"
              <c:if test="${themeInfo.subTheme eq 'idea' || themeInfo.subTheme eq 'ga'}"> data-txnmode="PAYTM_POSTPAID" onclick="pushGAData(this, 'pay_now_clicked')" </c:if>
                     value="Pay Now" style="width: 98%;border-radius: 5px;" name="">
            </c:when>
          </c:choose>
