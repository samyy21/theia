<%String contextPathSaved = com.paytm.pgplus.theia.utils.ConfigurationUtil.getProjectProperty("context.path"); %>
...
<div data-role="content" data-theme="i" id="savedcard-container" <c:if test="${'5' ne paymentType}">style="display:none"</c:if>>
	...
	<section>
		<form autocomplete="off" method="post" action="<%=contextPathSaved %>/payment/request/submit" data-ajax="false">
				...
				<c:forEach var="card" items="${savedCardList}" varStatus="count">
					...
					<div class="label" id="deleteDiv">
						...
						<c:if test="${!empty cardImagePrefix}">&nbsp;
							<img src="<%=contextPathSaved %>/images/wap/${cardImagePrefix}-card.jpg" title="" alt=""/>
						</c:if>
						<a href="#" onclick='$("#overlay").popup("open")' id="${card.cardId}" style="float: right; margin-right: 5px">
							<img src="<%=contextPathSaved %>/images/wap/delete.png" alt="Delete" title="Delete" />
						</a> 
					</div>
				</c:forEach>
			</div>
...
<script>
	$('#deletCardButton').live('click', function() {
		...
		$("#overlay").popup("close");
		window.location.href = "<%=contextPathSaved %>/DeleteCardDetails?savedCardId=" + cardId + "&ajax=no";
	}
	);
</script>