...
<head>
	...
	<script type = "text/javascript">
        ...
        // When insufficient Balance (Paytm || Paytm PostPaid || Paytm Bank Account)
...
// 	           }
        ...
        function onEMIBankSelect(select){
            var isHybridAllowedForEMI = "${txnConfig.hybridAllowed}" === "true",
                ...
                NONE = "none",
                url = "/theia/jsp/wap/merchantLow/paymentForm.jsp?txn_Mode=EMI&emi_bank=" + select.value,
        ...
        function bajajFinservChanges(){
            ...
            if(isEMIMode){
                ...
                if(!isBajajFinservBank && bankSelectEle && bankSelectEle[0]){
                 	isBajajFinservBank = bankSelectEle[0].value;
				}
...
<div>
	...
	<c:if test="${param.use_wallet eq 0}">
		...
		<c:if test="${empty param.txn_Mode && sessionScope.previousPaymentType ne 18}">
            ...
            <!-- reset paymentType only if param.txnMode empty and sessionScope.previousPaymentType not equal to 18 (upi ) -->
			<c:set var="paymentType" value="${sessionScope.previousPaymentType}"></c:set>
		</c:if>
	</c:if>
    ...
    <!-- END UPI PUSH SECTIONS -->


	<div class="header clear" id = "otherText">
		<c:choose>
			<c:when test="${paymentType eq 17 || paymentType eq 16}">
				<!-- when payment is fulfilled by either paytm or with payment bank -->
			</c:when>
