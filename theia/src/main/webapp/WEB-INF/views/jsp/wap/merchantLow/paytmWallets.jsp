    ...
    <div class="row mb5 rel_pos" <c:if test="${disablePaytmCashCheckbox}">style="color:#9a9999;"</c:if>>
      <div class="fl">
        <input name="paytmCashCB" id="paytmCashCB" type="checkbox" value="" onclick="processPaytmCash(this)" <c:if test="${checkPaytmCashCheckbox}">checked="checked"</c:if>  <c:if test="${disablePaytmCashCheckbox}">disabled="disabled"</c:if> />
					<span class="rel_pos span_inline" <c:if test="${disablePaytmCashCheckbox eq false}">onclick="walletItemClickable(this.previousElementSibling)"</c:if>>Paytm
...
<c:if test="${isPPBL_Enabled && themeInfo.subTheme ne 'ccdc'}">
  ...
  <c:set var="isPaymentBankCheckboxDisable" value="${!paymentBankInactive &&
			((usePaytmCash && ((walletInfo.walletBalance + (0 + paymentBankAccountBalance)) < txnInfo.txnAmount)) ||
  ...
  <c:if test="${(isPPBL_addMoneyEnable || isPPBL_merchantPayModeEnable) && !paymentBankInactive  &&
  ...
  <c:if test="${usePaytmCash && (isPPBL_addMoneyEnable || isPPBL_merchantPayModeEnable) && isPaymentBankCheckboxChecked}">
  ...
  <c:if test="${(usePaytmCash && (walletInfo.walletBalance >= txnInfo.txnAmount)) &&
			((isPPBL_addMoneyEnable || isPPBL_merchantPayModeEnable) && !paymentBankInactive && (0 + paymentBankAccountBalance >= txnInfo.txnAmount))}">
...
<%--  <c:if test="${(not empty param.use_wallet) && (param.use_wallet eq 0) && (not empty param.use_payment_bank) && (param.use_payment_bank eq 0) && (digitalCreditInfo.digitalCreditInactive eq false && digitalCreditInfo.digitalCreditEnabled eq true)}">
...
<c:if test="${loginInfo.loginFlag && (isPPBL_Enabled) && themeInfo.subTheme ne 'ccdc' }">
    <div class="row mb5 rel_pos ${isPPBL_show}" <c:if test="${paymentBankInactive eq true|| paymentBankRetryCount eq 0 || isInsufficientPaymnetBankBalance eq true}">style="color:#9a9999;"</c:if>>
      <div class="fl">
        <input name="paymentBankCheckbox" id="paymentBankCheckbox" type="checkbox" value=""
               onchange="processPaymentBank(this)"
               <c:if test="${paymentBankInactive}">disabled="disabled"</c:if>
        ...
        <form autocomplete="off"
          ...
          <div class="btn-submit fl" style="width: 100%;">
            <input class="gry-btn" type="submit"
                   ...
                   data-txnmode="PAYMENTBANK" onclick="pushGAData(this, 'pay_now_clicked')"
                   id="pbSubmit" />
          </div>
        </form>
        <div class="clear"></div>
        ...
        <div class="btn-submit fl" style="width: 100%;">
          <c:choose>
            <c:when test ="${digitalCreditInfo.passcodeRequired}">
              <input class="gry-btn" id="postSubmit" type="submit" value="Pay Now" data-txnmode="PAYTM_POSTPAID" onclick="pushGAData(this, 'pay_now_clicked')" style="width: 98%;border-radius: 5px;" name="">
            </c:when>

            <c:when test ="${!digitalCreditInfo.passcodeRequired}">
              <input class="blue-btn" id="postSubmit" type="submit" value="Pay Now"data-txnmode="PAYTM_POSTPAID" onclick="pushGAData(this, 'pay_now_clicked')"  style="width: 98%;border-radius: 5px;" name="">
            </c:when>
          </c:choose>
        </div>
      </form>
