	...
	<c:if test="${emiAvailable}">
		<div class="row-dot">
			...
			<c:if test="${!empty param.emi_bank && 'AMEX' eq param.emi_bank || emiSelectedBank eq 'AMEX'}">
				<div id="emiAmexConveFeeMsg" class="failure" style="font-size:12px;color:#222;padding:10px;margin-left:0;line-height:17px;width:90%; border:none; background:#f9ffcf;">2% processing fee would be charged by your bank for the first EMI. Taxes extra, as per bank terms and conditions.</div>
			</c:if>
...
<script>
	...
	function onEMIPlanSelect(checkbox){
		...
		location.href = "/theia/jsp/wap/merchantLow/paymentForm.jsp?txn_Mode=EMI&emi_bank=${emiSelectedBank}&emi_plan_id=" + emiValue + "&${queryStringForSession}";
