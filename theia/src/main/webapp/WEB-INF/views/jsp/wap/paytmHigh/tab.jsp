<select name="select1" id="paymentModes" data-icon="dwn-arw">
	...
	<c:if test="${cashcardEnabled}">
		<option value="itz" <c:if test="${'10' == paymentType }">selected=selected</c:if>>Cash Card</option>
	</c:if>
	<c:if test="${rewardsEnabled}">
		<option value="rewards" <c:if test="${'11' == paymentType }">selected=selected</c:if>>Rewards</option>
	</c:if>
</select>