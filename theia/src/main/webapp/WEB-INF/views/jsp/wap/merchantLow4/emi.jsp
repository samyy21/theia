	...
	<c:if test="${emiAvailable}">
		<div class="row-dot">
			...
			<c:if test="${!empty param.emi_bank && 'AMEX' eq param.emi_bank || emiSelectedBank eq 'AMEX'}">
				<div id="emiAmexConveFeeMsg" class="failure" style="font-size:12px;color:#222;padding:10px;margin-left:0;line-height:17px;width:90%; border:none; background:#f9ffcf;">2% processing fee would be charged by your bank for the first EMI.</div>
			</c:if>
			<br>
		...
		<div class='row-dot <c:if test="${empty emiSelectedPlanId}">hide</c:if>'>
			<p>
		    	...
		    	<input id="card-number" name="cardNumber" type="tel" maxlength="23" class="mt5 ${cssClass}" onblur="onCardNumberBlur()" onkeydown="pressCardSpace(event);" onkeyup="onCardDown(true);"/>
		    	${errorText}
		    ...
		    <p class="pt7 cvv">
		        ...
		        <input id="emiCVVBox" type="tel" maxlength="4" size="5" name="cvvNumber" class ="mask cvv-input ${cssClass}" onkeyup="onKeypressCvvNumber(arguments[0])"/> <img src="${ptm:stResPath()}images/wap/merchantLow/security-code.png" alt="Security Code" title="Security Code" />
		        ${errorText}
		    </p>
		    ...
		    <p class="pt7 ">
		    	<button type="submit" id="btnSubmit" class="blue-btn">Pay Now</button>
		    </p> 
