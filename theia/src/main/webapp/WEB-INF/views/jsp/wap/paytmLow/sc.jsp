<c:if test="${5 eq paymentType}">
		...
		<form autocomplete="off" method="post" action="/submitTransaction" onsubmit = "submitForm()">
		...
		<c:forEach var="card" varStatus="status" items="${savedCardList}">
			...
			<div class="row-dot sc-row ${cardState}" data-firstsixdigits="${card.firstSixDigit}">
				...
				<c:if test="${!empty cardImagePrefix}">
					<img src="/images/wap/paytmLow/${cardImagePrefix}.png" alt="" title="" />
				</c:if>
				<a href="theia/DeleteCardDetails?savedCardId=${card.cardId}&ajax=no" class="deleteCard">Delete</a>
				...
				<div class="pad cvv" style = "display:none" id = "cvv_${card.cardId}_pad">
					<p class="pad10">
						...
						<input type="password" maxlength="3" size="9" id = "cvv_${card.cardId}_input" class = "${cssClass}"/>
						<img src="/images/wap/paytmLow/cvv.png" alt="CVV" title="CVV" />
						${errorText}
					</p>
					<p class="pt7 ml20">
						<input type="submit" value="Proceed securely" class="blue-btn" />
					</p>
					<div class="pt7">
