	...
	<head>
		...
		function toggleCVVTopBottomIdebit(isDisplayed) {
            if (isDisplayed) {
				...
				if(document.getElementById('normalOptionCVVTxt')){
					document.getElementById('normalOptionCVVTxt').removeAttribute('disabled');
				}

				if(document.getElementById('idebitOptionCVVTxt')){
...
<body>
	...
	<div>
		...
		<%-- <c:if test="${empty hidePaytmCash}"> --%>	
			<div class="row pt20">
				...
				<div class="small mt10 mb5 hide" id="subs-details">
					...
					<span>Max. amount debited per subscription : <span class="WebRupee">Rs</span> ${txnInfo.subscriptionMaxAmount}</span>
				</div>
			</div>
		<%-- </c:if> --%>
		<%-- headers --%>
	   ...
	   <%-- promocode logic --%>
		...
		<c:if test="${(selectedModeType eq 'ADD_MONEY' or selectedModeType eq 'SUBS_FULL_WALLET' or selectedModeType eq 'SUBS_DEFAULT') && (selectedModeType eq 'ADD_MONEY' or selectedModeType eq 'SUBS_FULL_WALLET' or selectedModeType eq 'SUBS_DEFAULT') && ((txnInfo.subscriptionPaymentMode eq 'PPI' && walletInfo.walletBalance < txnInfo.txnAmount) || (txnInfo.subscriptionPaymentMode eq 'NORMAL'))}">
