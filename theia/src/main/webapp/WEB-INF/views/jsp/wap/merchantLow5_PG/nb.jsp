...
<c:if test="${3 eq paymentType}">
	<style>
		...
		input[type="radio"]:checked + span {
			font-weight: bold;
			border-bottom: 4px #56a00b solid;
			padding-bottom: 6px;
			...
			<form autocomplete="off" method="post" action="${pageContext.request.contextPath}/payment/request/submit?${queryStringForSession}" onsubmit = "return submitForm()">
				...
				<div class="row-dot">
					...
					<c:if test="${!empty nbList}">
						<!-- BANKS Tabs  -->
						<p class="row-dot" style="margin-left: 10px;width: 90%;">
							<c:forEach begin="0" end="5" step="1" varStatus="loopVar">
								<c:if test="${nbList.size() > loopVar.index}">
									...
									<label class="nbTab" onclick="nbBankSelect()" style="text-align:center;">
								<span style="background : url(${ptm:stPath()}native/bank/${bank.bankName}.png) no-repeat; no-repeat;
										display: inline-block;
										width: 40px;
										height: 40px;
										background-size: 100%;     margin-right: -22px;"></span>

										<input type="radio" class="bankNms" name="bankNm" id="${bank.bankName}" value="${bank.bankName}" <c:if test="${bank.bankName eq txnInfo.selectedBank}">checked</c:if> style="visibility:hidden;" />

								<span style="display: block;">${bank.bankName}</span>
									</label>
								</c:if>
							</c:forEach>
						</p>
						...
						<p class="pt7">
							<button type="submit" style="width:97%;margin-left:5px;border-radius: 5px;" class="clear gry-btn" disabled = "disabled" id="btnSubmit">Pay Now</button>
						</p>
					</c:if>
				</div>
			</form>
