...
<script type="text/javascript">
	...
	function atmSelectChange(param) {
		...
		var value = param.options[selectedIndex].value;
		var cardNumError = document.getElementById("cardnum-error");
		if(cardNumError) {
			cardNumError.style.display = "none";
		}
		var cardExpiryError = document.getElementById("cardexpiry-error");
		if(cardExpiryError) {
			cardExpiryError.style.display = "none";
		}
		var cardCVVError = document.getElementById("cardcvv-error");
		if(cardCVVError) {
			cardCVVError.style.display = "none";
		}
		if(value.indexOf("other", value.length - "other".length) !== -1) {
			document.getElementById("debitcard-div").style.display = "block";
		} else {
			
			document.getElementById("debitcard-div").style.display = "none";
		}
...
boolean selectOther = false;
	...
	boolean isNokiaAsha = false;
	if(userAgent != null && userAgent.contains("Nokia311")) {
		isNokiaAsha = true;
	}
%>
<body>
<c:if test="${txnInfo == null}">
	...
	<div class="header">
		...
		<div class="fr">
			<img src="images/wap/logo.png" alt="Paytm" title="Paytm" 
				<%if(isNokiaAsha){%>
					height="21" width="105" style="margin-top:3px"
				<%} %>
			/>
		</div>
		<div class="clear"></div>
	...
	<c:if test="${ ('5' ne paymentType) }">
		<div class="border-bar">
			<div class="fl">
				<a href="jsp/wap/<%=theme %>paymentForm.jsp?txn_Mode=SC"> Saved Cards </a>
			</div>
			<div class="fr">
	...
	<c:if test="${ccEnabled}">
		<c:if test="${'1' ne paymentType }">
			<div class="border-bar">
				<div class="fl">
					<a href="jsp/wap/<%=theme %>paymentForm.jsp?txn_Mode=CC"> Credit<%if(isNokiaAsha){%>/Debit<%} %> Card </a>
				</div>
				<div class="fr">
					<img src="images/wap/arrow.png" />
				</div>
		...
		<c:if test="${'1' eq paymentType }">
			<div class="blu-hd">
				<div class="fl">Credit<%if(isNokiaAsha){%>/Debit<%}%> Card</div>
				<div class="fr">
					<img src="images/wap/dwn-arw.png" alt="" title="" />
				</div>
				<div class="clear"></div>
			...
			<div class="form-container">
				<form autocomplete="off" method="post" action="/payment/request/submit">
					<p class="mt10">
						<%if(isNokiaAsha){%>Credit/Debit<%} %> card no. <br /> <input autocomplete="off" type="text"
	...
	<c:if test="${dcEnabled || atmEnabled}">
		<c:if test="${'2' ne paymentType && '8' ne paymentType}">
			<div class="border-bar">
				<div class="fl">
					<a href="jsp/wap/<%=theme %>paymentForm.jsp?txn_Mode=DC"> Debit Card </a>
				</div>
				<div class="fr">
					<img src="images/wap/arrow.png" />
				</div>
		...
		<c:if test="${'2' eq paymentType || '8' eq paymentType}">
				...
				<div id="debitcardwap-container">
					<p class="mt10">
						...
						<c:if test="${!empty entityInfo.completeATMList}">
							<c:forEach var="item" items="${entityInfo.completeATMList}">
           ...
           <div id="debitcard-div" style="display:none">
					...
					<c:if
						...
						<p class="error" id = "cardnum-error">${requestScope.validationErrors['INVALID_CARD']}</p>
					</c:if>
					<p class="mt10">
					...
					<c:if
						...
						<p class="error" id = "cardexpiry-error">${requestScope.validationErrors['INVALID_CARD_EXPIRY']}</p>
					</c:if>
					<p class="mt10">
					...
					<c:if
						...
						<p class="error" id = "cardcvv-error">${requestScope.validationErrors['INVALID_CVV']}</p>
					</c:if>
					<c:if test="${saveCardOption}">
					<p class="mt10">
						<label class="sm-txt"><input name="storeCardFlag" type="checkbox" value="true" />&nbsp;Save this card for future transactions</label>
					</p>
					</c:if>
			</div>		
				</div>	
	...
	<c:if test="${netBankingEnabled}">
		<c:if test="${'3' ne paymentType}">
			<div class="border-bar">
				<div class="fl">
					<a href="jsp/wap/<%=theme %>paymentForm.jsp?txn_Mode=NB"> Net Banking </a>
				</div>
				<div class="fr">
					<img src="images/wap/arrow.png" />
				</div>
			...
			<c:if test="${!empty requestScope.validationErrors && !empty requestScope.validationErrors['INVALID_BANK']}">
				...
				<form autocomplete="off" method="post" action="/payment/request/submit">
					...
					<p class="mt10">
						...
						<input type="hidden" name="txn_Mode" value="NB" /> <input
...
<% if(showOthers){%>
	document.getElementById("debitcard-div").style.display="";
<%} %>
