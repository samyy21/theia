		 ...
		 function processPaytmCash(checkbox) {
			...
			if (checkbox.checked) {
					...
					if(addMoney){
						...
						if($("#CC-form").length && !isSubsPPIOnly){
...
<body>
	...
	<div>
			...
			<div id="showHideWallet" class="${!empty hidePaytmCash ? 'hide' : ''}">	
				<div class="row mb5 rel_pos">
					<div class="fl">
						...
						<div class="bal" id = "yourBal" >(Your current balance is  <span class="WebRupee">Rs</span> <span class = 'amt' id = "yourBalSpan"><fmt:formatNumber value="${walletInfo.walletBalance}" maxFractionDigits="2" /></span>)</div>
					    <input type = "hidden" value = "${walletInfo.walletBalance}" id = "totalWalletVal"/>
					</div>
					<c:if test="${usePaytmCash}">
						<div class="fr abs_pos" id = "walletBalance">
							 - <span class="WebRupee">Rs</span>
							 <span id = "walletBalanceAmt"><fmt:formatNumber value="${walletInfo.walletBalance}" maxFractionDigits="2" /></span>
						</div>
					</c:if>
	   ...
	   <div class="header" id = "otherText">
	   		<span id="balance">Select an option to complete payment for subsequent subscription transactions.<c:if test="${usePaytmCash}">balance</c:if></span> <span class="fr ${!empty hidePaytmCash || usePaytmCash ? '' : 'hide'}">Rs <span id = "balanceAmt"><fmt:formatNumber value="${txnInfo.txnAmount}" maxFractionDigits="2" /></span></span>
		</div>
		
		<div class="header row hide" id="subs-select-card">
			<span>Select and save your <b class="b">Credit Card</b> for subsequent transactions.</span>
		</div>
		
		<div class="header row hide" id="subs-save-card">
			<span>Save your <b class="b">Credit Card</b> for subsequent transactions.</span>
	   ...
	   <div class="header row hide" id="subs-ppi-only-complete-payment">
			<b class="fr">Rs <span id = "addMoneyAmt"><fmt:formatNumber value="${txnInfo.txnAmount}" maxFractionDigits="2" /></span></b>
