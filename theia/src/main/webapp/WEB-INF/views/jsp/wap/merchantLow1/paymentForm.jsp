		...
		<script type = "text/javascript">
		 ...
		 function processPaytmCash(checkbox) {
			...
			if (checkbox.checked) {
					...
					$("#walletBalance")[0].style.display="none";
					$("#yourBalSpan")[0].innerHTML=Math.round(walletBalance * 100) / 100;
					walletAmountToUse = 0;
					// show user balance
		 ...
		 function showAllPaymentModes(){
			 location.href = "/theia/jsp/wap/merchantLow1/paymentForm.jsp?showAll=1" + "&${queryStringForSession}";
			 return;
		 };
// 		 var verifier={
//          	verified: false,
...
// 	           }
		 ...
		 function onEMIBankSelect(select){
			 location.href = "/theia/jsp/wap/merchantLow1/paymentForm.jsp?txn_Mode=EMI&emi_bank=" + select.value + "&${queryStringForSession}";
		 }
		 
		 function onEMIPlanSelect(checkbox){
			 location.href = "/theia/jsp/wap/merchantLow1/paymentForm.jsp?txn_Mode=EMI&emi_plan_id=" + checkbox.value + "&${queryStringForSession}";
		 }
...
<body>
	...
	></div>
	<%-- <c:set  var="loginUrl" value="${loginInfo.oAuthInfoHost}/oauth2/authorize?response_type=code&scope=paytm&theme=${loginInfo.loginTheme || 'pg')}&client_id=${sessionScope.oauth_info.client_id}&redirect_uri=${sessionScope.oauth_info.return_url}" ></c:set> --%>
