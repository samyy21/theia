...
<c:if test="${txnInfo == null}">
	...
	<c:if test="${saveCardEnabled}">
		<c:if test="${ ('5' ne paymentType) }">
			<div class="border-bar">
				<div class="fl">
					<a href="jsp/wap/merchant/paymentForm.jsp?txn_Mode=SC"> Saved Cards </a>
				</div>
				<div class="fr">
					...
					Select your card<br /> 
							...
							<c:forEach var="card" varStatus="status" items="${savedCardList}">
								...
								<li id="${card.cardId}-item" >
									<label><input class="hidden" type="radio" name="savedCardId" <c:if test="${status.index==0}">checked='checked' </c:if> value="${card.cardId}"/>Use ${card.cardNumber} </label><c:if test="${!empty cardImagePrefix}">&nbsp;<img src="images/wap/${cardImagePrefix}-card.jpg" alt="" title=""/></c:if>
	...
	<c:if test="${ccEnabled}">
		<c:if test="${'1' ne paymentType }">
			<div class="border-bar">
				<div class="fl">
					<a href="jsp/wap/merchant/paymentForm.jsp?txn_Mode=CC"> Credit Card </a>
				</div>
				<div class="fr">
					<img src="images/wap/arrow.png" />
				</div>
	...
	<c:if test="${dcEnabled || atmEnabled}">
		<c:if test="${'2' ne paymentType && '8' ne paymentType}">
			<div class="border-bar">
				<div class="fl">
					<a href="jsp/wap/merchant/paymentForm.jsp?txn_Mode=DC"> Debit Card </a>
				</div>
				<div class="fr">
					<img src="images/wap/arrow.png" />
				</div>
	...
	<c:if test="${netBankingEnabled}">
		<c:if test="${'3' ne paymentType}">
			<div class="border-bar">
				<div class="fl">
					<a href="jsp/wap/merchant/paymentForm.jsp?txn_Mode=NB"> Net Banking </a>
				</div>
				<div class="fr">
					<img src="images/wap/arrow.png" />
				</div>
