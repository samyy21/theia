...
<%-- 				</c:if> --%>
            		...
            		<c:if test="${!card.iDebitCard}">
						<p class="pad10 cvv-box">
							...
							<c:if test="${card.cardScheme eq 'SBIME' || card.cardScheme eq 'MAESTRO'}">
								<input type="tel" disabled name="cvvBox" maxlength="${isIMPS ? '8' : '4'}" size="9" id = "cvv_${card.cardId}_input" autocomplete="off" class ="mask cvv-input ${isIMPS ? 'medium-input' : ''} ${cssClass}" onkeyup="onKeypressCvvNumber(arguments[0]); removeFieldError(null, this)"/>
							</c:if>
		...
		var isPostConvEnabled = document.getElementById('payment-details') && document.getElementById('payment-details').getAttribute('data-postconv-enabled');
		...
		if(cardId) {
		 ...
		 function setCVV() {
			    if(document.getElementById("cvvNum") && document.getElementById(openedCardId + "_input")){
