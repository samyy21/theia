	...
	<c:if test="${sarvatraVpainfo.key eq kycBankInfo.key }">
			...
			<div class="mb10 clear">
                ...
                <c:if test="${!empty param.UPI_KEY && !empty param.txn_Mode && param.txn_Mode eq 'UPI_PUSH'}">
                    ...
                    <c:set var="upiPushcheckBoxSelected" value="${sarvatraVpainfo.key eq upiPushSelected && !isRetryAvaiable}"></c:set>
...
<script>
   ...
   if(paymentType== 18 && !getParameter('txn_Mode') && "${savingsAccountInfo.invalidPassCodeMessage}"==="" && (getParameter('use_wallet') != 1 && ("${isPPBL_Enabled && (paymentBankInactive eq true)}" ==='true' && parseFloat(walletBalance) >= parseFloat("${txnInfo.txnAmount}"))  || parseFloat(walletBalance) < parseFloat("${txnInfo.txnAmount}")) && getParameter('use_paytmcc') !== "1" && "${isRetryAvaiable}"==="false"){
...
if(upiPushCheckedBox && UPIPush_VPA){
    ...
    for(var i=0; i< pushBtns.length;i++){
     ...
     var upiLocation= upiPushRadioBtn.getAttribute("data-href");
      ...
      if(!upiPushRadioBtn.checked){
          ...
          }else if("${digitalCreditInfo.digitalCreditEnabled}" ==="true" && "${digitalCreditInfo.digitalCreditInactive}" === "false"){
              // postpaid enabled
  ...
  var upiPushFormSubmitted = false;
   ...
   // Call from IOS App in UPI Push flow
  function isIos_UPI_Push() {
      isIOS = true;
      return isIOS;
  }
   ...
   var upiPushInput =inputBtn.parentElement.parentElement.querySelector("input[name=upiPush]");
    ...
    if(window['HtmlOut']){
        ...
        if(isIOS){
               // Call IOS App flow
            var iosPayload = {
                "amount":upiPushTxnAmt,
                "payload":payeeData,
                "payeeVPA":upiPushMIDInputVal
            };
            var scheme = "HtmlOut_IOS";
            var host = "onPayNowClick_IOS";
            try{
                document.location.href = scheme + '://' + host + "#" + escape(JSON.stringify(iosPayload));
            }catch (e){}
           }else {
               // Collect flow
  ...
  function setMpinAndSubmit(mpin,deviceId,sequenceNo,App_Id){
    if((mpin && mpin!= undefined) && (deviceId && deviceId!= undefined) ){
    var mpinTxt=document.getElementById("mpin");
    var device=document.getElementById("deviceId");
    var sequenceNumber=document.getElementById("sequenceNumber");
    var App_Id_Elm=document.getElementById("appId");
