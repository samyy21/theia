...
<c:if test="${isUPI_Push}">
    ...
    <c:if test="${loginInfo.loginFlag  && empty param.txn_Mode && empty param.use_wallet && empty param.use_paytmcc && walletInfo.walletBalance < txnInfo.txnAmount && !isRetryAvaiable}">
...
// 		  };
                ...
                if(radioflag ==1)
                {
                    ...
                    if(cvvElm)
                    {
                       ...
                       // isCCCvvValid = (((cvv) && !isNaN(cvv) && (cvv.length == 3 || cvv.length == 4))  || (curSavedCard.querySelector("div[id=CARDSCHEME]").getAttribute("data-value")=="MAESTRO"));
                        isCCCvvValid = (((cvv) && !isNaN(cvv) && (cvv.length == cvvL))  || (curSavedCard.querySelector("div[id=CARDSCHEME]").getAttribute("data-value")=="MAESTRO" || curSavedCard.querySelector("div[id=CARDSCHEME]").getAttribute("data-value")=="BAJAJ" || curSavedCard.querySelector("div[id=CARDSCHEME]").getAttribute("data-value")=="BAJAJFN"));
        ...
        function validateCCForm(){
            ...
            if(form.elements['card-number'] && !(form.elements['card-number'].classList.contains("maestro") || form.elements['card-number'].classList.contains("bajaj") || form.elements['card-number'].classList.contains("bajajfn"))) {
                //Expiry Month
                var monthElm = form.elements['ccExpMnth'];
                var month = monthElm.value;
                var isCCExpMonthValid = month == "0" ? false : true;

                if (form.elements['card-number'].classList.contains("maestro") || form.elements['card-number'].classList.contains("bajaj") || form.elements['card-number'].classList.contains("bajajfn")) {
            ...
            {
                var cvv = cvvElm.value;
                isCCCvvValid = (((cvv) && !isNaN(cvv) && (cvv.length == cvvElm.maxLength)) || ( form.elements['card-number'].classList.contains("maestro") || form.elements['card-number'].classList.contains("bajaj") || form.elements['card-number'].classList.contains("bajajfn") ) ) ? true : false;
            }
            else
                isCCCvvValid = false;
            if(!isCCCvvValid)
        ...
        function validateDCForm(){
            ...
            if(cvvElm)
            {
                ...
                if((!isNaN(cvv) && cvv.length==cvvElm.maxLength) || ( form.elements['card-number'].classList.contains("maestro") || form.elements['card-number'].classList.contains("bajaj") || form.elements['card-number'].classList.contains("bajajfn")))
                    isDCCvvValid=true;
                else
                    isDCCvvValid = false;
            }
        ...
        function validateEmiCCForm() {
            ...
            {
                var cvv = cvvElm.value;
                isEMICvvValid = (((cvv) && !isNaN(cvv) && (cvv.length == cvvElm.maxLength)) || ( form.elements['card-number'].classList.contains("maestro") || form.elements['card-number'].classList.contains("bajaj") || form.elements['card-number'].classList.contains("bajajfn"))) ? true : false;
            }
        ...
        var ppblNpp_submitted=false;
        function ppblNpp_submit(form){
            if(!form){
                return false;
            }
            if(ppblNpp_submitted)
                return false;
        ...
        function changeCardTypeIcon(value, formNm) {
            ...
            var allIcons = ['master','maestro','visa','discover','bajajfn','bajaj','rupay','amex','diners','INVALID_CARD']
...
// 	           }
        ...
        function showAuthView(type, open) {
            ...
            var merchantDetail = document.getElementById("merchantDetail");
            var params = [];
            ...
            if(merchantDetail && merchantDetail.getAttribute){
                var merchantName = merchantDetail.getAttribute("data-name");
                if(merchantName){
...
<div>
	...
	<c:if test="${ loginInfo.loginFlag && empty usePaytmCash}">
		...
		<c:if test="${(!empty txnInfo.promoCodeResponse && !empty txnInfo.promoCodeResponse.promoCodeDetail.promoCode) && (!txnConfig.addMoneyFlag && !walletInfo.walletOnly)}">
...
<!-- Google Tag Manager (window Load) -->
<script type = "text/javascript">
    ...
    window.onload = function(e) {
      /*  if(dataLayer){
            dataLayer.push({
                "event" : "payment_gateway_load",
                "theme":"merchantLow5_GA",
                "order_id" : '${txnInfo.orderId}',
                "mid" : '${txnInfo.mid}',
                "user_agent" : navigator.userAgent,
                "vendor" : 'idea',
            });

            //Push whether useris LoggedIn
            if("${loginInfo.loginFlag}" == "true") {
...
<!-- End Google Tag Manager (window Load) -->

<!-- PUSHING BACKEND ERRORS IN GA -->
<c:if test="{!empty errorText}">
    <script>pushGAData(false, 'exception_visible', "${errorText}");</script>
</c:if>
<c:if test="${!empty requestScope.validationErrors['INVALID_PROMO_DETAILS'] }">
    <script>pushGAData(false, 'exception_visible', "${requestScope.validationErrors['INVALID_PROMO_DETAILS']}");</script>
</c:if>
