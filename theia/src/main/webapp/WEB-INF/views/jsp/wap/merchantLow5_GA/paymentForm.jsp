...
// 		  };
        ...
        function validateDCForm(){
            ...
            if(binObject && binObject.info && binObject.info.binDetail && binObject.info.binDetail.iDebitEnabled && form.elements['isIciciIDebit'][1].checked) {
                cvvElm = form.elements['idebitOptionCVVTxt'];
            } else if(binObject && binObject.info && binObject.info.binDetail && binObject.info.binDetail.iDebitEnabled) {
                cvvElm = null;
            }
...
// 	           }
        ...
        function bajajFinservChanges(){
            var isEMIMode = location.search.search("txn_Mode=EMI") !== -1,
                cvvBlock,cvvInp,isBajajFinservBank,DEFAULT_CVV_VALUE,
                emiBankDropDownElm = document.getElementById("emiBankSelect");

            if(isEMIMode){
                cvvBlock = document.getElementsByClassName("cvv");
                cvvInp = document.getElementsByClassName("cvv-input");

                if(emiBankDropDownElm){
                    isBajajFinservBank = emiBankDropDownElm.value === "BAJAJFN";
                }

                DEFAULT_CVV_VALUE = "111";

                if(cvvInp && cvvInp.length > 0){
                    // set cvv input to default when bajaj finserv is selected
                    cvvInp[0].value = isBajajFinservBank ? DEFAULT_CVV_VALUE : "";

                    // set element to disabled
                    if(isBajajFinservBank){
                        cvvInp[0].disabled = true;
                    }
                }
...
<div>
    ...
    <c:if test="${isUPI_Push && isUpiPush_show}">
        ...
        <c:if test="${loginInfo.loginFlag  && empty param.txn_Mode && param.use_wallet eq 1 && param.use_payment_bank eq 0 && paymentType eq 18 && walletInfo.walletBalance  < txnInfo.txnAmount}">
            <c:set var="paymentType" value="2"></c:set>
        </c:if>
...
<script type = "text/javascript">
    ...
    <script type = "text/javascript">
        ...
        function pushGAData(ele, actionType){

            var authConfig=JSON.parse($("#auth-config")[0].getAttribute('data-value'));
            var txnMode=$(ele)[0].getAttribute('data-txnmode');
