...
<div class="card content ${3 eq paymentType ? 'active' : ''}" id="nb-card">
	...
	<%
		...
		java.util.Map<String, java.util.HashMap<String, Object>> paymentCharges = (java.util.Map<String,java.util.HashMap<String, Object>>)request.getSession().getAttribute("paymentCharges");
	%>
	<form autocomplete="off" name="netbanking-form" method="post" action="${pageContext.request.contextPath}/payment/request/submit?${queryStringForSession}" style="padding: 0px" class="validated">
		...
		<c:if test="${!empty nbList}">
						...
						<c:if test="${nbList.size() > loopVar.index}">
							...
							<li>
								<div id="${bank.bankName}" title="${bank.bankName}" class="radio" style="background-position: center 0px;">
									...
									<label class="fl">
										<span class="bank-logo" alt="${bank.bankName}" style="background : url(${ptm:stResPath()}images/web/bank/${bank.bankWebLogo}) no-repeat;background-size: 112px 26px;"></span>
									</label>
								</div>
							</li>
						</c:if>
						...
						<c:forEach var="bank" items="${nbList}" begin="0">
							<option
									<c:if test="${! empty txnConfig.paymentCharges}">
										data-bm='${txnConfig.paymentCharges.NB.baseTransactionAmount}'
										data-ct='${txnConfig.paymentCharges.NB.text}'
										data-tm='${txnConfig.paymentCharges.NB.totalTransactionAmount}'
										data-cf='${txnConfig.paymentCharges.NB.totalConvenienceCharges}'
									</c:if>
									value="${bank.bankName}" ${bank.bankName eq txnInfo.selectedBank ? "selected='selected'" : ""}  data-bankid="${bank.bankId }">
									${bank.displayName}
							</option>
							<script type="text/javascript">
								<c:if test="${bank.maintainence}">
			...
			<div id="warningDiv" class="hide clear" style="margin-top: 15px;    width: 400px !important;    line-height: 1.45;">
			...
			<div class="mt20">
				<div class="btn-submit ${submitBtnClass} fl">
					<button name="" type="submit" class="gry-btn blue-btn required btn-normal btn-submit" id = "nbSubmit" data-txnmode="NB" onclick="pushGAData(this, 'pay_now_clicked')">${submitBtnText}</button>
