...
<div class="container">
	...
	<c:choose>
	    <c:when test="${!empty phone1}">
	      	<c:set var="phone" value="${phone1 }"/>
	    </c:when>
	    <c:when test="${!empty phone2}">
	...
	<c:set var="loginData" value="${orderid}:${mid}:${txnTransientId}:${detectedChannel}:${jsessionid}:${loginType}:${eid}"/>
	
	<br>
	<!-- phone not empty -->
	<c:if test="${!empty phone}">
		<div class="message-area" style="display: inline-block;margin: 20px 15px;">
			<p id="myContent" style="display:none;float: left;font-size: 17px;line-height: 1.4;color: rgb(102, 102, 102);">Please confirm the mobile number to receive the One Time Password to login to Paytm</p>
		  	<p id="enter-otp" style="float: left; font-size: 17px; line-height: 1.4; color: rgb(74, 74, 74);">Enter the One Time Password received on <a id="open-change-mobile-frame" style="font-size: 17px;color: #00B9F5;cursor: pointer;text-decoration: underline;" onclick="openFrame('loginData=${loginData }&phone=&redirectUri=${returnUrl }&clientId=${clientId}')">${phone }</a> to login to Paytm</p>
			
		</div>
		<div class="iframe-container" id="iframe-container-1" style="position: relative;border-radius: 10px;overflow: hidden;">
			<iframe id="frame-1" src="${host}/oauth2/login/otp?loginData=${loginData }&phone=${phone }&redirectUri=${returnUrl}&clientId=${clientId}" height="200px" width="100%"></iframe>
		</div>
	</c:if>
	...
	<c:if test="${empty phone}">
		...
		<div class="message-area" style="display: inline-block;margin: 20px 15px;">
			<p id="myContent" style="float: left;font-size: 17px;line-height: 1.4;color: rgb(102, 102, 102);">Please confirm the mobile number to receive the One Time Password to login to Paytm</p>
			...
			<p id="enter-otp" style="display:none; float: left; font-size: 17px; line-height: 1.4; color: rgb(74, 74, 74);">Enter the One Time Password received on <a id="open-change-mobile-frame" style="font-size: 17px;color: #00B9F5;cursor: pointer;text-decoration: underline;" onclick="openFrame('loginData=${loginData }&phone=&redirectUri=${returnUrl }&clientId=${clientId}')">abcs</a> to login to Paytm</p>
		</div>

		<div class="iframe-container" style="position: relative;border-radius: 10px;overflow: hidden;">
			<iframe id="frame-1" src="${host}/oauth2/login/otp?phone=${phone }&loginData=${loginData }&redirectUri=${returnUrl }&clientId=${clientId}" height="200px" width="100%" name="add-mobile" id="add-mobile"></iframe>
		</div>
	</c:if>
<p style="
    ...
    margin-left: 15px;">By proceeding you accept the terms and conditions mentioned at <a href="https://paytm.com/terms" target="_blank" style="
    color: #01bdf2;
    font-size: 10px;
">www.paytm.com/terms</a></p>
</div>
