	...
	<div class="card-header xsmall grey-text mb10 relative">
		<div class="fr">
			<div class="secure lt-grey-text">
	        	...
	        	<span><i>Your payment details are secured via 128 Bit encryption by Verisign</i></span>
	        </div>
		</div>
		<div class="clear"></div>
	</div>

	<form autocomplete="off" name="creditcard-form" method="post" action="${pageContext.request.contextPath}/payment/request/submit?${queryStringForSession}" id="dc-form" class="dc-form validated">
		...
		<input type="hidden" name="CARD_TYPE"  value="${txnInfo.cardType}" />
		...
		<ul class="grid">
        	<li class="mb20 card-wrapper">
            	...
            	<c:if test="${'2' eq paymentType && errorsPresent && !empty requestScope.validationErrors['INVALID_CARD']}">
            		<c:set var="dcErrorInputClass">error1</c:set>
            	</c:if>
                ...
                <c:if test="${dcErrorInputClass eq 'error1'}">
					<div id="dcCCnoId" class="error error2 clear" style="width: 150px;">
            ...
            <li class="ml10 fr relative" id = "dcCvvWrapper">
...
<%-- 		<c:if test="${entityInfo.iDebitEnabled}"> --%>
			<div id="idebitPayOption" class="mt20 hide card">
            	
                <div class="idebitOption" style="margin-left:0;">
                   	<label>
                   		<input type="radio" class="checkbox idebitDC" name="isIciciIDebit"  value="Y" checked>
                   			Use ATM PIN
                   	</label>
                </div>
                <div class="idebitOption" >
                	<label>
                		<input type="radio" class="checkbox idebitDC"  name="isIciciIDebit"  value="N">
 ...
 <%--        </c:if>  --%>
		...
		<div id = "cardInfo" class="storeCardWrapper">
			<div class="txt12 fl opt mt10" id = "maestroOpt" style = "display:none;">If your Maestro Card does not have Expiry Date/CVV, skip these fields</div>
