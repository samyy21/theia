	...
	<form autocomplete="off" name="creditcard-form" method="post" action="${pageContext.request.contextPath}/payment/request/submit?${queryStringForSession}" id="card" class="cc-form validated">
		...
		<ul class="grid">
        	<li class="mb20 card-wrapper">
            	...
            	<p class="cd">
					...
					<input autocomplete="off" type="text" name="" class="ccCardNumber ${ccErrorInputClass} text-input large-input c cardInput type-tel card-field-selector" id="cn" size="16" maxlength="23" style="width: 278px" data-type="cc"
				...
				<div class="error error2 promo-code-error <c:if test='${empty promoError && empty ccErrorInputClass}'>hide</c:if>">
					...
					<c:if test="${ccErrorInputClass eq 'error1'}">
						<c:if test="${!empty requestScope.validationErrors['INVALID_CARD']}">
							${requestScope.validationErrors['INVALID_CARD']}
							<script>
									pushGAData(false, "exception_visible", "${requestScope.validationErrors['INVALID_CARD']}");
							</script>
						</c:if>
						<c:if test="${!empty requestScope.validationErrors['INVALID_BIN_DETAILS']}">
							${requestScope.validationErrors['INVALID_BIN_DETAILS']}
							<script>
									pushGAData(false, "exception_visible", "${requestScope.validationErrors['INVALID_BIN_DETAILS']}");
							</script>
						</c:if>
						<c:if test="${!empty requestScope.validationErrors['INVALID_INTERNATIONAL_CARD']}">
							${requestScope.validationErrors['INVALID_INTERNATIONAL_CARD']}
							<script>
									pushGAData(false, "exception_visible", "${requestScope.validationErrors['INVALID_INTERNATIONAL_CARD']}");
							</script>
						</c:if>
						<c:if test="${!empty requestScope.validationErrors['INVALID_CARDSCHEME']}">
							${requestScope.validationErrors['INVALID_CARDSCHEME']}
							<script>
									pushGAData(false, "exception_visible", "${requestScope.validationErrors['INVALID_CARDSCHEME']}");
							</script>
						</c:if>
					</c:if>
		...
		<c:if test="${saveCardOption}">
	        ...
	        <div class="fl" id="ccSaveCardLabel">
				...
				<c:if test="${isSubscriptionFlow eq true}">
					<input type="hidden" style="display:none;" name="storeCardFlag" checked="checked" value="Y">
				</c:if>
			</div>
			<c:if test="${isSubscriptionFlow eq true}">
        ...
        <div class="mt20">
        	<div class="btn-submit ${submitBtnClass} fl">
        		<button name="" type="submit" class="gry-btn btn-normal btn-submit" id = "ccSubmit"  data-txnmode="CC" onclick="pushGAData(this, 'pay_now_clicked')">${submitBtnText}</button>
<%-- 	           	<input name="" type="submit" class="gry-btn btn-normal" value="${submitBtnText}" id = "ccSubmit"> --%>
	         </div>
