<div class = 'card content ${13 eq paymentType ? "active" : ""}' id = "emi-card">
	<div class="card-header xsmall grey-text mb10 relative">
		<div class="fr">
			<div class="secure lt-grey-text">
				...
				<span><i>Your payment details are secured via 128 Bit encryption by Verisign
				</i></span>
			</div>
		</div>
		<div class="clear"></div>
	...
	<div class="fl">
		<form autocomplete="off" class="emi-form validated" name="emi-form" method="post" action="${pageContext.request.contextPath}/payment/request/submit?${queryStringForSession}" id="emi-form">
			...
			<c:if test="${!empty param.emiBankName}">
				...
				<c:if test="${fn:length(emiInfoList) eq 1}">
					<c:set var="emiSelectedBank" value="${emiInfoList[0].bankName}" scope="session" />
			...
			<c:forEach var="i" begin="1" end="2">
				...
				<div class="emi-bank-map emi-bank-map-${i} ${i eq 2 ? ' hide' : ''}">
					...
					<c:if test="${emiAvailable}">
						...
						<div <c:if test="${emiDisabledBank eq true}"> style="opacity:0.5;position: relative;"</c:if>>
							<c:if test="${emiDisabledBank eq true}">
								<div class="overlay" style="position: absolute;z-index: 9;width: 100%;left: 0;top: 0;height: 51px;"></div>
							</c:if>
			...
			<div class="card-details-wrapper mt20 hide">
				<ul class="grid">
		        	<li class="mb20 card-wrapper">
		            	...
		            	<p class="cd">
							...
							<input type="hidden" name="cardNumber" value="${CardNumberWithoutFormatting} class="required" />
							 --%>
							<input autocomplete="off" type="text" name="" class="emiCCCardNumber ${ccErrorInputClass} text-input large-input c cardInput type-tel card-field-selector" id="cn" size="16" maxlength="23" style="width: 278px" data-type="cc"
								value="${defaultCardNumberFormatted}"> 
		            ...
		            <li class="fl expiry-wrapper">
		     	    	<label class="mb10 ccExpMonth ccExpYear emiExpMonth" for = "emiExpMonth">EXPIRY DATE</label>
		               	<div class="mb10">
		               		...
		               		<c:if test="${'13' eq paymentType && errorsPresent  && !empty requestScope.validationErrors['INVALID_CARD_EXPIRY']}">
			                	...
			                	<select class="ccExpMonth ${ccDatesErrorClass} combobox required" id="emiExpMonth" name="expiryMonth" style="width: 80px;">
		               		...
		               		<div class="fl ml10" id = "emiExpYearWrapper">
		            ...
		            <li class="ml10 fr" id = "ccCvvWrapper">
		                ...
		                <div class="cvv-block">
		                	...
		                	<input class="ccCvvBox ${ccCvvErrorClass} text-input small-input required type-tel" autocomplete="off" type="password" name="cvvNumber"  maxlength="4" id="emiCvvNumber">
		                	<div class="clear"></div>
