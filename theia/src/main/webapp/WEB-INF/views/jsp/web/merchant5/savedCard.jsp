...
<c:set var="isFirstChecked" value="false"></c:set>
	...
	<form autocomplete="off" name="savecard-form" method="post" class="form-horizontal validated" action="${pageContext.request.contextPath}/payment/request/submit?${queryStringForSession}" id="card" class="sc-Form">
				...
				<div class="card-header small grey-text mb20">
           		...
           		<div class="controls" id="one">
             		<div class="input-prepend grid">
               			<span class="add-on fl">
                 			<label class="radio cvvRadio">
                   				<input type="checkbox" name="cno" class="sc-checkbox checkbox" id="${card.cardId}" data-firstsixdigits="${card.firstSixDigit}" data-txnMode="${card.txnMode}" test="card.count"  <c:if test="${!isFirstChecked}"> checked </c:if> <c:if test="${! empty txnConfig.paymentCharges}"> data-cType="${card.paymentMode}"  </c:if>    />
					...
					<c:if test="${card.iDebitCard}">
				                <div id="idebitSavedCard" class="mt10 ml50 idebitSavedCard" style="display:none;">
				...
				<div class="control-group  card ${!empty txnInfo.promoCodeResponse && !empty txnInfo.promoCodeResponse.promoCodeDetail.promoCode && !txnInfo.promoCodeValid ? 'hide' : ''}">
					...
					<div class="controls" id="one">
						<div class="input-prepend grid">
               			<span class="add-on fl">
                 			<label class="radio cvvRadio">
								<input type="checkbox" name="cno" class="sc-checkbox checkbox" id="${card.cardId}" data-firstsixdigits="${card.firstSixDigit}" test="card.count"  <c:if test="${!isFirstChecked}"> checked </c:if> <c:if test="${! empty txnConfig.paymentCharges}"> data-cType="${card.paymentMode}"  </c:if>  />
								<c:if test="${!isFirstChecked}">
									<c:set var="isFirstChecked" value="true"></c:set>
								</c:if>
							</label>
						...
						<span class="add-on fl">
							<label class="radio cvvRadio">
								<input type="checkbox" name="cno" class="sc-checkbox checkbox"
									...
									test="card.count"
										<c:if test="${! empty txnConfig.paymentCharges}"> data-cType="${card.paymentMode}"  </c:if>
										<c:if test="${!isFirstChecked}"> checked </c:if> />
								<c:if test="${!isFirstChecked}">
									<c:set var="isFirstChecked" value="true"></c:set>
								</c:if>

								<c:if test="${!empty txnConfig.paymentCharges.IMPS}">
									<div class="irctcData" id="impsCard"
										 data-bm="${txnConfig.paymentCharges.IMPS.baseTransactionAmount}"
										 data-ct="${txnConfig.paymentCharges.IMPS.text}"
										 data-tm="${txnConfig.paymentCharges.IMPS.totalTransactionAmount}"
										 data-cf="${txnConfig.paymentCharges.IMPS.totalConvenienceCharges}"
											>	</div>
								</c:if>
						...
						id="cvvDiv-${card.cardId}" style="display:none;">
		               	<ul class="grid">
		               		<li>
				                   ...
				                   <label for="submit-btn-${card.cardId}" class="mb7">${inputLabel}</label>
									<c:if test="${card.cardScheme eq 'SBIME' || card.cardScheme eq 'MAESTRO'}">
										<p><input type="text" disabled name="otp" id="cvv-${card.cardId}" class="width40 scCvvInput text-input ${isIMPS ? 'medium' : 'small'}-input type-tel" maxlength="${isIMPS ? '8' : '4'}" minlength="${isIMPS ? '6' : '3'}" value=""></p>
									</c:if>
									<c:if test="${card.cardScheme ne 'SBIME' && card.cardScheme ne 'MAESTRO'}">
