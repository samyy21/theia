...
$(document).ready(function() {
	...
	var paytmCardBalance=$("#totalPaytmtCardVal").val();
	var isPageFirstLoad = true;
	if(paytmCardBalance){
		paytmCardBalance = Math.round(paytmCardBalance * 100) / 100;
	}

	var isOTPLoginAvailable = $("#isOtpLoginAvailable").length ? true : false;

	var isIrctcAvailableDIV = $("#paymentCharges").length ? true : false;
	if (isIrctcAvailableDIV){
	...
	// For Wallet Click
	var walletClick=false;

	// Payment Bank

	var paymentBank=$("#paytmBank");
	if($("#paytmBank").length>0) {
		var paymentBankEnabled = paymentBank.data("bankenabled");
		var paymentBankInactive = paymentBank.data("bankinactive");
		var paymentBankBalance = paymentBank.data("paytmbankbalance");
		var bankPassCodeInvalidMsg = paymentBank.data("paytmpasscdemsg");
	}

	var isPPBLE_AddMoney=$("#isPPBLE_AddMoney").data("value");
	var isPPBLE_merchantPaymodes=$("#isPPBLE_merchantPaymodes").data("value");
	...
	var useWallet = false;
	var usePaytmCard = false;
	var usePaytmBank=false;
    var paytmCardAvailable=$("#paytmCard").data("usepaytmcard");
    var paytmCardInactive=$("#paytmCard").data("inactivepaytmcard");
	if(paytmCardAvailable){
		var paytmCCErrorAvailable=$("#paytmCC-errorMsg").data("iserror");
		var prevPaymentMode=$("#paytmCC_prevPaymentMode").data("prevpaymode");
	}
	if(paytmCardAvailable && !paytmCardInactive){
		usePaytmCard=true;
	}
	if($("#paytmBank").length>0){
		if(!paymentBankInactive){
			usePaytmBank=true;
		}
	}
	...
	var isScCheckboxTriggeredForIdebit = false;

    var isSignup = $("#isSignup");
    var isSignUpVal = false;
    if(isSignup){
        isSignUpVal=isSignup.data("issignup");
	}
	...
	$("body").on('click','.cards-control .card',function(e){
		...
		var walletDetails = $("#wallet-details");
		var addMoneyDetails = $('#addMoney-details');
		if(walletDetails && addMoneyDetails){

			var addMoneyAvailable = addMoneyDetails.data("available");
			var isWalletOnly = walletDetails.data("wallet-only");
			if(isWalletOnly && addMoneyAvailable && $(this).hasClass("blured")){
				//unblurAllModes();
				if($("#paytm-paymentBank-checkbox").length>0){
					$("#paytm-paymentBank-checkbox").checkbox("click");
				}
			}
		}
...
//		}
        ...
        if(contentId == "#cod-card"){
            $("#paytmWalletCheckButton .blur-overlay").show();
            if($("#paytm-cash-checkbox").length > 0 && $("#paytm-cash-checkbox")[0].checked){
          		  $("#paytm-cash-checkbox").checkbox("click");
            }
            
        } else {
            $("#paytmWalletCheckButton .blur-overlay").hide();
        ...
        // set cvv to empty on tab change
        var cvvInp = document.getElementById("ccCvvBox"),
            cvvWrapper = document.getElementById("ccCvvWrapper");
        ...
        // show cvv block if tab click
        if(cvvWrapper){
            cvvWrapper.style.display = "block";
        }
	...
	function setupPromocode(){
		...
		if(promocodeAvailable){
			...
			promocode = {
				...
				promoBins:elm.data("promobins")
			};

			// hide paytm cash
			$(".payAmount").addClass("hide");
			$(".walletSwitchBox").addClass("hide");
		...
		if(promocode.type == "DISCOUNT") {
			if(_.indexOf(promocode.paymentModes, 'PPI') != -1){
				...
				$(".walletSwitchBox").removeClass("hide");
			}
		}
...
//				});
            ...
            $.ajax({
                ...
                success: function(data, status, jqXHR) {
					if ('success' == data) {
						...
						if(checkSavedCardsCount()){
							checkSavedCardsCount();

							// to select first card after delete
							if($('input[name=cno]') && $('input[name=cno]').length > 0){
								$($('input[name=cno]')[0]).trigger("click");
							}
						} else {
							if($("#merchant-payment-modes").length>0){
								$("#merchant-payment-modes #leftTab .grid li:not(.hide)").eq(0).click();
							}
						}
					}
	...
	$(".scCvvInput").on("keyup", function(){
		...
		var valid;
		if(card.find(".sc-icon").hasClass("MAESTRO-sc") || card.find(".sc-icon").hasClass("BAJAJ-sc") || card.find(".sc-icon").hasClass("BAJAJFN-sc")){
			if(cvv=='' || !isNaN(cvv)){
				valid = true;
			} else {
				valid = false;			
	...
	var listOfPossibleCardTypes = ['DEBIT_CARD','CREDIT_CARD','UPI'];
	//type is either CREDIT_CARD or DEBIT_CARD etc.

	var modeYetNotSetForSC = false;
	...
	//	if(isIdebitEnabled){
	 	...
	 	$("input.sc-checkbox").on("click change", function(event){
             event.stopPropagation();

			 if(isScCheckboxTriggeredForIdebit && $(this).closest(".control-group") && $(this).closest(".control-group").hasClass("active")) {
             	event  && event.preventDefault();
             	return;
             }else if(!isScCheckboxTriggeredForIdebit){
             	isScCheckboxTriggeredForIdebit = true;
             }
	         ...
	         if($(this)[0].checked){
	        	 ...
	        	 //console.log('inside',bankName,cardSchemeName,cardSchemeLowSuccessRate,issuerLowSuccessRate);
	        	 ...
	        	 if(issuerLowSuccessRate || cardSchemeLowSuccessRate){
					 ...
					 $('#warningDiv').show();
					 pushGAData(false, 'exception_visible', errorMsg);
	         ...
	         if($(this)[0].checked) {
	        	 if (isIdebitEnabled == "true" || isIdebitEnabled == true){
					 formNm.find("#idebitSavedCard").removeClass("hide");
					 formNm.find(".paymentIdebit").removeAttr("disabled");

	                 // also hide cvv
	                 $(this).parents("div.control-group").find('.saveCardCvvDiv').hide();
	        	 } else {
					 formNm.find("#idebitSavedCard").addClass("hide");
					 formNm.find(".paymentIdebit").attr("disabled","disabled");
	                 // also show cvv
	                 $(this).parents("div.control-group").find('.saveCardCvvDiv').show();
	        	 }
	         }
	 	});

    $('#add-money-payment-modes #idebitSavedCard, #merchant-payment-modes #idebitSavedCard').on("click change", function() {
		var form= $(this).parents("form");
	 		...
	 		if (elementSelected && elementSelected.val() == 'Y') {
				...
				form.find("input.sc-checkbox:checked").parents('.control-group').find('.saveCardCvvDiv').hide();
	 		} else {
	 			form.find("input.sc-checkbox:checked").parents('.control-group').find('.saveCardCvvDiv').show();
	 		}
	 	});
		 ...
		 $("input.idebitDC").on("click",function(){
			 ...
			 //if(isIdebitEnabled){
		         ...
		         //TODO disable validations on dc form if cvv is hidden
			 //}
		 });
	...
	$("input.sc-checkbox").on("change", function(e) {
        ...
        setTimeout(function () {
            ...
            if (checkbox.checked) {
                ...
                // show cvv box
                var isIdebitEnabled = form.find(".control-group.card.active .issuerIdebit").text();

                if(isIdebitEnabled=="false") {
                    form.find("#cvvDiv-" + saveCard).show();
                }

                else {
					if((mode!="hybrid-bank" || mode!="full-paymentBank") && mode !="addMoney-bank"){
						if(form.find('#idebitSavedCard').length > 0) {
                            if(isPageFirstLoad){
                                // On page load idebit is click so payment bank uncheck
                                // to restrict that from happening return when page is first loaded
                                // so it didn't click on idebit element
                                isPageFirstLoad = false;
                                return;
                            }
							form.find('#idebitSavedCard').click();
                        }
					}
                }
                ...
                if (form.hasClass("impsForm")) {
                    form.parent(".savedCardSection").find(".savedCardbtn").addClass("hide");
...
//Check EMI CC Card valid/invalid 
	...
	$(".emiBankSelect").on("change", function(){
		...
		var elm = form.find("." + selectedBank + "-bank");
		form.find("." + selectedBank + "-bank").removeClass("hide");
...
//			form.find('#dcStoreCardWrapper').hide();
	...
	// PAYTM CC FORM VALIDATION

	$("body").on("click",".paytmCCDeduct .btn-normal", function(){
		var form = $(this).closest("form");
		var txtPassFld = form.find("#txtPassCode");

		if(!txtPassFld) return true;

		var passcodeVal=txtPassFld.val();

		if(passcodeVal.length>0 && passcodeVal.length>=4 &&  passcodeVal.length <=6){
			form.find("#txtPassCode").removeClass("error1").focus();
			return true;
		}
		else{
			form.find("#txtPassCode").addClass("error1").focus();
			return false;
		}
	...
	$("body").on("click",".paymentBank .btn-normal", function(){
		var form = $(this).closest("form");

		var passcodeVal=form.find("#banktxtPassCode").val();

		if(passcodeVal.length>0 && passcodeVal.length>=4){
			form.find("#banktxtPassCode").removeClass("error1").focus();
			return true;
		}
		else{
			form.find("#banktxtPassCode").addClass("error1").focus();
			return false;
		}
	...
	function validateDCForm(){
        ...
        if(!binDetail || (binDetail && binDetail.cardName && (binDetail.cardName != "MAESTRO" || binDetail.cardName != "BAJAJ" || binDetail.cardName != "BAJAJFN"))) {
            monthElm.toggleClass("valid", isDCExpMonthValid);
            yearElm.toggleClass("valid", isDCExpYearValid);
        } else if(binDetail && binDetail.cardName && (binDetail.cardName == "MAESTRO" || binDetail.cardName == "BAJAJ" || binDetail.cardName == "BAJAJFN")) {
...
//		var isDCCvvValid = (!isNaN(cvv) && (cvv.length == 3 || cvv.length == 4)) ? true  : false;
		...
		if(cardNumber.length >= 6) {
           ...
           // var clueBox = form.find(".cvv-clue-box");
			...
			if (cardType == "maestro" || cardType == "bajaj" || cardType == "bajajfn") {
	//			form.find('#dcStoreCardWrapper').hide();
...
//	 }
		...
		if(value.length>=6){
            ...
            if(!binObject.binData[value] && (binObject.firstSix.length == 0 || value != binObject.firstSix)){
                ...
                $._post("/theia/bin/fetchBinDetails", {
			     	...
			     	"ORDER_ID":$("#sucess-rate").attr("data-oid").toString(),
					"txnMode": txnMode
			    },
    ...
    function enableMaestroFields(enableMaestro, formNm) {
        ...
        var cvvFld = formNm.find("#ccCvvBox, #dcCvvBox, #scCvvBox");
    ...
    function validateCardOnSubmit(form) {
        if ($(form).is("#card.cc-form") || $(form).is("#card.dc-form") || $(form).is("#card.emi-form")) {
            ...
            if(inputFld) {
                ...
                if (CardName != "MAESTRO" && CardName != "NONE" && CardName != "BAJAJ" && CardName != "BAJAJFN" && (yearFld.val() == "0" || monthFld.val() == "0" || ((yearFld.val() + monthFld.val()) < currDate))) {
                ...
                if(cvvFld && !isDCCvvValid && cvvFld[0] && CardName != "MAESTRO" && CardName != "NONE" && CardName != "BAJAJ" && CardName != "BAJAJFN" && !isBajajFin) {
	...
	function changeCardTypeIcon(value, formNm) {
        ...
        if(formNm && formNm.length > 0) {
            ...
            var allIcons = ['master', 'maestro', 'visa', 'discover', 'bajajfn', 'bajaj','rupay', 'amex', 'diners', 'INVALID CARD'];
            ...
            if (formNm.hasClass('dc-form') || formNm.hasClass('cc-form') || formNm.hasClass('emi-form')) {
                ...
                if (currentCardType != "INVALID CARD") {
                    ...
                    if (clueBox) {
                        ...
                        else if(currentCardType != "amex" && currentCardType != "maestro" && currentCardType != "bajaj" && currentCardType != "bajajfn")
						{
	...
	function cardLowSuccesRateBin(value,formNm){
		if(value.length>=6 && binObject && binObject.info && (binObject.info.issuerLowSuccessRate || binObject.info.cardSchemeLowSuccessRate)){
			...
			pushGAData(false, 'exception_visible', lowSuccessRateMsg);
		} else {
	...
	function iciciIdebitBin(value,formNm){
	 ...
	 || binObject.info.binDetail.iDebitEnabled == "true")) {
		...
		// hide show CVV & disabled it's validations

	}


	// only numeric cvv
	$("input[name=cvvNumber], .scCvvInput, #txtPassCode, #banktxtPassCode").on("keypress", isNumber);
...
//		form.find('#ccStoreCardWrapper').show();
	...
	function validateEmiCCForm(){
		...
		var form = $(this);
        var bankNameElm = form.find("select.emiBankSelect");
		var monthElm = form.find('select.ccExpMonth');
		var month = monthElm.val();
		var isCCExpMonthValid = month == "0" ? false : true;
		monthElm.toggleClass("valid", isCCExpMonthValid);
        ...
        if(bankNameElm && bankNameElm.length > 0 && bankNameElm[0].value === "BAJAJFN"){
            // bajaj finserv changes
            cvvElm.toggleClass("valid",true);
        }else{
            cvvElm.toggleClass("valid", isCCCvvValid);
        }
...
/*        if(cardNum && cardNum.length >= 6 && !(binObject.info.binDetail && binObject.info.binDetail.cardName)) {
        ...
        if(form.attr('name')=='savecard-form'){
			...
			if($(cvv).parents(".saveCardCvvDiv").css("display")==="block" && !isSubscriptionSCValid(activeCard[0])){
                ...
                //for maestro,bajaj and bajajfn cvv is not required
                if(activeCard.find(".sc-icon").hasClass("MAESTRO-sc") || activeCard.find(".sc-icon").hasClass("BAJAJ-sc") || activeCard.find(".sc-icon").hasClass("BAJAJFN-sc")){
	...
	$("body").on('click','.savedCardbtn', function(e){
		...
		var cvvVal=cvv.val();
		if($(cvv).parents(".saveCardCvvDiv").css("display")==="block"){
...
//			}
    ...
    $("#cn, #cn1, #rewardsCardNo, #ccCvvBox, #dcCvvBox, #scCvvBox").on("keyup", function(e){
        var $target = $(e.target);
        if($target[0].className.split(' ').indexOf("cardInput") != -1)
	...
	function checkLowSuccessRate(value){
		//console.log('oinside low success rate:' + value);
		value = value.replace(' ','');
		if(value.length>=6){
			var url="/theia/bin/fetchBinDetails?bin="+value.substring(0,6);
	//         if(!verifier.verified || (verifier.firstSixDigit!=cardNo.substring(0,6))) {
			$.get(url, function(res){
				//console.log('result:',res);
				//the response is returned as string
	...
	if(promocodeAvailable){
		...
		$('#cn, #cn1').on('blur',function() {
			...
			// BIN list
			var promoBins = promocode.promoBins;
...
$("#neftNo").on("keyup", validateNEFTForm);
	...
	$('.netbanking-panel input.bankRadio').on("change", function(e){
		...
		if(maintainenceNBBank[bankName]) {
			...
			pushGAData(false, 'exception_visible', displayName + " " + maintenanceMsg);
...
//	}
	...
	function getAuthUrl(type,open){
        ...
        if(merchantDetail){
        	var merchantName = merchantDetail.data('name');
        	if(merchantName){
                params.push('client_name=' + merchantName);
			}
		}
...
//		console.log($("#auth-config"));
		...
		var logoutUrl = authConfig.AUTH_LOGOUT_URL;
		
		// var params = [];
		var returnUrl = authConfig.AUTH_LOGOUT_RETURN_URL + "?" + $("#queryStringForSession").data("value");
		// params.push('redirect_uri=' + returnUrl);
		
		var src = logoutUrl + "?XSRF-TOKEN=" + $("#auth-js-details").data("userid");
...
function rupayCard(cardNo){
	...
	function isClickOnOtherPayModes(e){
		return $(e.target).parents("#overlayBox").length > 0;
	}
	...
	function clickOnBlured(e){
		...
		if($("#paytmCard").data("usepaytmcard") && event &&  !isClickOnOtherPayModes(event) && $(event.target).attr("id")!="paytm-cash-checkbox" && $("#paytm-digitalCard-checkbox")[0].checked){
			...
			if (!$(e.target).hasClass('checkbox') && $("#merchant-payment-modes").find("#leftTab").find(".grid .card").hasClass("blured")) {
				if ($("#paytm-cash-checkbox").length > 0 && $("#paytm-cash-checkbox")[0].checked && ($("#paytm-paymentBank-checkbox").length > 0 && !paymentBankInactive && $("#paytm-paymentBank-checkbox")[0].checked)){
					//$("#paytm-paymentBank-checkbox").checkbox("click");

                    if(!isClickOnOtherPayModes(event)){
                        if(hybridPaymentAllowed || addMoneyAvailable) {

                            if ($("#paytm-paymentBank-checkbox").length > 0) {
                                $("#paytm-paymentBank-checkbox").checkbox("click");
                                $("#paytm-cash-checkbox").checkbox({checked: true});
                            }
                            ...
                            if ($("#paytm-paymentBank-checkbox").length > 0) {
                                $("#paytm-paymentBank-checkbox").checkbox("click");
                                $("#paytm-cash-checkbox").checkbox({checked: false});
                            }

                        }
                    } else {
                        if ($("#paytm-paymentBank-checkbox").length > 0 && !paymentBankInactive && $("#paytm-paymentBank-checkbox")[0].checked) {
                            $("#paytm-paymentBank-checkbox").checkbox("click");
                        }
                        if ($("#paytm-digitalCard-checkbox").length > 0 && !paytmCardInactive && $("#paytm-digitalCard-checkbox")[0].checked) {
                            $("#paytm-digitalCard-checkbox").checkbox("click");
                        }
                    }

				}
				else{
					if ($("#paytm-cash-checkbox").length > 0 && $("#paytm-cash-checkbox")[0].checked && e.target.id !== "idebitSavedCard") {
						$("#paytm-cash-checkbox").checkbox("click");
					}
					if ($("#paytm-paymentBank-checkbox").length > 0 && !paymentBankInactive && $("#paytm-paymentBank-checkbox")[0].checked) {
						$("#paytm-paymentBank-checkbox").checkbox("click");
    ...
    function addMoneyUnBlurOtherPayModes() {
        var overlay = $("#add-money-payment-modes .top-overlay");
        if(overlay){
	...
	function initPayments(){
		...
		$("body").on('change', '#paytm-cash-checkbox', function(e){
			...
			walletClick=true;
			if(useWallet && $("#paytmCard").data("usepaytmcard") && !hybridPaymentAllowed){
				$("#paytm-digitalCard-checkbox").checkbox({checked:false});
				usePaytmCard=false;
			}

			if(hybridPaymentAllowed || addMoneyAvailable){

				if($("#paytm-paymentBank-checkbox").length > 0 && $("#paytm-paymentBank-checkbox")[0].checked){
					$("#paytm-paymentBank-checkbox").checkbox({checked:true});
					usePaytmBank=true;
				}

			}
			else if(!hybridPaymentAllowed && !addMoneyAvailable && !useWallet && !paymentBankInactive){
				...
				if(useWallet && $("#paytm-paymentBank-checkbox").length>0){
					$("#paytm-paymentBank-checkbox").checkbox({checked:false});
					usePaytmBank=false;
				}
			}
		...
		$("body").on('change', '#paytm-digitalCard-checkbox', function(e){

            if($(".cod-wallet-disable").length>0 && $(".cod-wallet-disable")[0].style.display=="block"){
                $(".cod-wallet-disable").hide();
            }

			usePaytmCard = $("#paytm-digitalCard-checkbox")[0].checked;
			if(usePaytmCard){
                if($("#paytm-cash-checkbox").length>0) {
                    if(hybridPaymentAllowed) {
                        //$("#paytm-cash-checkbox").checkbox({checked: true});
                        //useWallet = true;
            ...
            if(usePaytmCard && $("#merchant-payment-modes a[href='#cod-card']").length>0 && $("#merchant-payment-modes a[href='#cod-card']").parent().hasClass("active")){
			...
			// when user uncheck his Paytm Postpaid
            if(!$("#paytm-digitalCard-checkbox")[0].checked && $("#paytm-cash-checkbox").length>0 && $("#paytm-cash-checkbox")[0].checked &&  $("#merchant-payment-modes a[href='#cod-card']").length>0 && $("#merchant-payment-modes a[href='#cod-card']").parent().hasClass("active"))
            {
                $("#paytm-cash-checkbox").checkbox({checked: false});
                useWallet = false;
            }
		...
		$("body").on('change', '#paytm-paymentBank-checkbox', function(e){

            if($(".cod-wallet-disable").length >0 && $(".cod-wallet-disable")[0].style.display=="block"){
                $(".cod-wallet-disable").hide();
            }

			usePaytmBank = $("#paytm-paymentBank-checkbox")[0].checked;
			if(usePaytmBank){
				...
				if($("#paytm-digitalCard-checkbox").length>0){
					$("#paytm-digitalCard-checkbox").checkbox({checked:false});
					usePaytmCard=false;
				}
				...
				if($("#paytm-digitalCard-checkbox").length>0 && (walletAvailable && paytmCardBalance >= txnAmount)) {
					if(!usePaytmBank) {
						$("#paytm-digitalCard-checkbox").checkbox({checked:true});
						usePaytmCard=true;
						//$("#paytm-cash-checkbox").checkbox({checked: true});
						//useWallet = true;
		...
		if(addMoneyAvailable && walletAvailable && isSubscription === undefined){
			useWallet = true;
		} else if(walletAvailable && walletBalance>0 && isSubscription === undefined){
            // use wallet by default (if available)
			useWallet = true;
		}else if(walletAvailable && (isSubscription !== undefined && isSubscription === true && typeof walletBalance === "number")){
			useWallet = true;
		}else if($("#paytmCard").data("usepaytmcard")){
			usePaytmCard=true;
		}
		else {
			// disable paytmcash
	...
	*/
	...
	// end of post conv fee for all modes
	...
	function processPayments() {
		...
		// identify mode
		mode = "";
		if((useWallet || usePaytmCard || usePaytmBank) && !(retryOn)) {

			if(!paytmCardAvailable && !paymentBankEnabled){
				if(useWallet && walletBalance >= totalAmount) { // sufficient balance
				...
				if(useWallet && walletBalance >= totalAmount) { // sufficient balance
				...
				else if(!useWallet && (usePaytmBank && paymentBankBalance >= totalAmount) && isPPBLE_merchantPaymodes) { // sufficient balance
					mode = "full-paymentBank"; // full wallet payment
                    if(isSubscription && subsMode == "PPBL"){
                        mode = "subs-full-paymentBank";
                    }

				}
				else if(usePaytmCard && paytmCardBalance >= totalAmount && !useWallet){
					mode = "full-paytmCard"; // full wallet payment
					if(isSubscription){
						mode = "subs-full-paytmCard";
					}
					...
					else if(addMoneyAvailable && !paymentBankInactive && walletBalance+paymentBankBalance >= totalAmount && usePaytmBank && isPPBLE_AddMoney && useWallet){
						mode = "addMoney-bank"; // wallet + Paytm Payment Bank with add money
					}
					else if(addMoneyAvailable){
						mode = "add-money"; // wallet with add money
					}
					else if(hybridPaymentAllowed  && !paymentBankInactive && walletBalance+paymentBankBalance >= totalAmount && usePaytmBank && walletBalance > 0 && isPPBLE_merchantPaymodes && useWallet){
						mode = "hybrid-bank"; // hybrid payment
					}
                    else if(hybridPaymentAllowed  && paytmCardAvailable && !paytmCardInactive && walletBalance+paytmCardBalance >= totalAmount && usePaytmCard && walletBalance > 0){
                        mode = "hybrid-paytmPostpaid"; // hybrid with paytm postpaid
                    }
					else if(hybridPaymentAllowed && walletBalance > 0){
						mode = "hybrid"; // hybrid payment
					} else {
		...
		// issue below paytm digital ICICI CARD
        ...
        $(".PPBL").addClass("hide");
        if(isPPBLE_merchantPaymodes){
            $(".merchantPayModePPBL_Bank").removeClass("hide");
        }
		...
		switch(mode){
			case "default" :
                var walletDetailsEle = $("#wallet-details");
                var isWalletOnlyBool = walletDetailsEle.data("wallet-only");

                if(!(isWalletOnlyBool && window.prnEnabled)){
                	unblurAllModes();
				}
				...
				$("#paymentCreditCardBox").addClass("hide");

				// error paytm CC is available
				if(paytmCCErrorAvailable && $("#txtPassCode").hasClass("error1")){
					$("#txtPassCode").removeClass("error1");
					$("#paytm-digitalCard-checkbox").click();
				}

				if($("#paytm-cash-checkbox").attr("disabled")=="disabled"){
					$(".no-walletTextUpdate").removeClass("hide");
					$("#no-walletTextUpdate").siblings(".blur-overlay").show();
				}
				else{
					$(".no-walletTextUpdate").addClass("hide");
					$("#no-walletTextUpdate").siblings(".blur-overlay").hide();
				}

				if($("#paymentBank").length>0){
					$("#paymentBank").addClass("hide");
				if($("#add-money-payment-modes .blur-overlay").length>0){
						$("#add-money-payment-modes .blur-overlay")[0].style.display="none";
						$("#add-money-payment-modes .blur-overlay")[0].style.opacity="1";
				}

				}

				if(walletBalance==0 && !addMoneyAvailable){
					disablePaytmCash();
				}
                $("#merchant-payment-modes .cards-control .grid .active").click();
		...
		break;
			...
			case "no-wallet" :
				...
				$("#paymentCreditCardBox").addClass("hide");
				setTimeout(function(){
					$('.walletCheckbox').find(".bootstrap-checkbox").find(".cb-icon-check-empty").show();
					$('.walletCheckbox').find(".bootstrap-checkbox").find(".cb-icon-check").hide();
				}, 500);
				if($("#paymentBank").length>0) {
					$("#paymentBank").addClass("hide");
				}
			...
			case "full-wallet" :
				...
				bindOnceBlur();
				if($("#paytm-paymentBank-checkbox").length>0){
				if($("#paytm-paymentBank-checkbox")[0].checked){
					$("#paytm-paymentBank-checkbox").click();
				}
				}
				// error paytm CC is available
				if($("#paymentBank").length>0) {
					if (bankPassCodeInvalidMsg && $("#banktxtPassCode").hasClass("error1")) {
						$("#banktxtPassCode").removeClass("error1");
						$("#paytm-paymentBank-checkbox").click();
						return;
					}
				}
				...
				remPaytmCardBalance=paytmCardBalance - totalAmount;
				...
				$("#remPaytmCardBal .amt").text(Math.round(paytmCardBalance * 100) / 100);
				...
				if(isIrctcAvailable){
					$("#usedWalletAmt").text(numberWithCommas(totalAmount));
				}

				break;

			case "full-paymentBank":
				blurAllModes();
				bindOnceBlur();

				paidAmount = totalAmount;
				remPaymentBankBalance = paymentBankBalance - totalAmount;
				remPaytmCardBalance=paytmCardBalance - totalAmount;
				$('input[name=walletAmount]').val(totalAmount);
				$('.fullWalletDeduct').removeClass("f-hide");
				$('.balance-used-box, .fullWalletText').removeClass("hide");

				$("#remWalletBal .amt").text(numberWithCommas(Math.round(walletBalance * 100) / 100));
				$("#remPaymentBankBal .amt").text(numberWithCommas(Math.round(remPaymentBankBalance * 100) / 100));
				if($("#walletAmountPPBL") && $("#walletAmountPPBL").length > 0){
					$("#walletAmountPPBL")[0].value=0;
				}
				$("#fullWalletPayTxt").addClass("hide");
				$("#defultPayTxt").addClass("hide");
				$(".exactPayment").addClass("hide");
				$("#paymentBox").hide();
				$("#hybridPaymentBankTxn").addClass("hide");

				$("#walletAmountRem").addClass("hide");
				$("#walletAmountRem .walletAmountRemTxt").text(numberWithCommas(walletBalance));
				$("#paymentBank").removeClass("hide");
				$("#paymentCreditCardBox").addClass("hide");
				$("#sufficientBank").removeClass("hide");
				if($("#paytm-paymentBank-checkbox").length>0) {
					if (!$("#paytm-paymentBank-checkbox")[0].checked) {
						$("#paytm-paymentBank-checkbox").click();
					}
				...
				if($("#paymentBank").length>0){
					if($("#addNpay-PaymentBank").length >0){
						$("#addNpay-PaymentBank").attr("disabled","disabled");
					}
					if($("#hybrid-PaymentBank-walletBal").length >0){
						$("#hybrid-PaymentBank-walletBal").attr("disabled","disabled");
					}
				}
				if(walletBalance==0 && !addMoneyAvailable){
					disablePaytmCash();
			...
			case "full-paytmCard":
				...
				$('.fullWalletDeduct').removeClass("f-hide");
				$('.balance-used-box, .fullWalletText').removeClass("hide");
                $("#sufficientDigitalBal").removeClass("hide");
				$("#remWalletBal .amt").text(numberWithCommas(Math.round(walletBalance * 100) / 100));
				$("#remPaytmCardBal .amt").text(numberWithCommas(Math.round(remPaytmCardBalance * 100) / 100));
				$("#fullWalletPayTxt").addClass("hide");
				$("#defultPayTxt").addClass("hide");
				$(".exactPayment").addClass("hide");
				$("#paymentBox").hide();
                $("#hybridDigitalBalTxn").addClass("hide");

				$("#walletAmountRem").addClass("hide");
				$("#walletAmountRem .walletAmountRemTxt").text(numberWithCommas(walletBalance));
				$("#paymentCreditCardBox").removeClass("hide");
				if($("#paymentBank").length>0) {
					$("#paymentBank").addClass("hide");
				}

				if(!$("#paytm-digitalCard-checkbox")[0].checked){
					$("#paytm-digitalCard-checkbox").click();
				}
                if(walletBalance==0 && !addMoneyAvailable){
                    disablePaytmCash();
                ...
                if($("#paymentCreditCardBox").length>0){
                    if($("#hybrid-PaytmPostpaid-walletBal").length >0){
                        $("#hybrid-PaytmPostpaid-walletBal").attr("disabled","disabled");
                    }
                }
				break;
			case "hybrid-bank":
					...
					paidAmount = walletBalance+totalAmount;
					$(".addMoneyAmount,.hybridMoneyAmount").text(numberWithCommas(Math.round(paidAmount * 100) / 100));
					...
					$("#paymentBank").removeClass("hide");
					$("#sufficientBank").addClass("hide");
					$("#hybridPaymentBankTxn").removeClass("hide");
					var remPaytmBankBalance = paymentBankBalance-(totalAmount-walletBalance);
					$("#remPaytmBankBal .amt").text(numberWithCommas(Math.round(remPaytmBankBalance * 100) / 100));
				    $("#paymentBankBal").text(numberWithCommas(Math.round((totalAmount-walletBalance) * 100) / 100));
				...
				$(".hybridMoneyAmount").text(numberWithCommas(totalAmount));
				...
				$("#walletAmountRem").addClass("hide");
				$("#paymentCreditCardBox").addClass("hide");
				$("#paytmDigitalCard").parent().find(".blur-overlay").show();
				...
				if($("#paymentBank").length>0){
					if($("#addNpay-PaymentBank").length >0){
						$("#addNpay-PaymentBank").removeAttr("disabled");
					}
					if($("#hybrid-PaymentBank-walletBal").length >0){
						$("#hybrid-PaymentBank-walletBal").removeAttr("disabled");
					}
				}

				break;

            case "hybrid-paytmPostpaid":
                blurAllModes();
                bindOnceBlur();
                ...
                $("#paymentCreditCardBox").removeClass("hide");
                $("#sufficientDigitalBal").addClass("hide");
                $("#hybridDigitalBalTxn").removeClass("hide");
                var remDigitalCardBalance = paytmCardBalance -(totalAmount-walletBalance);
                $("#remDigitalCreditBal .amt").text(numberWithCommas(Math.round(remDigitalCardBalance * 100) / 100));
                $("#digitalCreditBal").text(numberWithCommas(Math.round((totalAmount-walletBalance) * 100) / 100));
                ...
                $("#remWalletBal-postpaid .amt").text(numberWithCommas(0));
                $("#fullWalletPayTxt").addClass("hide");
                $("#defultPayTxt").removeClass("hide");
                $(".exactPayment").removeClass("hide");
...
/*                if($("#paymentBank").length>0){
                ...
                if($("#paymentCreditCardBox").length>0){
                    if($("#hybrid-PaytmPostpaid-walletBal").length >0){
                        $("#hybrid-PaytmPostpaid-walletBal").removeAttr("disabled");
                    }
                }
                break;
            ...
            case "hybrid" :
				...
				if($(".emiBankSelect").length>0){
					$(".emi-bank-map-2").find("#"+$(".emiBankSelect").val()+"-emi-plans").find("input[name=emiPlanId]").eq(0).checkbox("click");
				}
                ...
                if($("#retryPaymentMode").data("value")=="emi"){
                    $(".emi-bank-map:not(.hide)").find(".emiBankSelect.selected").change();
                }
			...
			case "addMoney-bank":
				if(isPPBLE_AddMoney){
					$(".addMoneyPPBL_Bank").removeClass("hide");
				}
				blurAllModes();
				bindOnceBlur();
				...
				if($("#paymentBank").length>0){
					...
					$("#add-money-payment-modes").addClass("blured");

					$("#add-money-payment-modes .blur-overlay")[0].style.display="block";
					$("#add-money-payment-modes .blur-overlay")[0].style.opacity="0";
			...
			case "add-money" :
				...
				if($("#paymentBank").length>0){
					...
					$("#add-money-payment-modes").removeClass("blured");
                ...
                if(isIrctcAvailable){
                    $(".ir-addMoney").show();
                    $(".ir-Money").hide();
                    $(".totalPcfAmt").eq(0).text(numberWithCommas(totalAmount));
                    $(".totalPcfAmt").eq(1).text(numberWithCommas(totalAmount));
                }
			...
			case "subs-full-paymentBank":
                ...
                $("#paymentBank").find(".blur-overlay").hide();
                break;
		}
	...
	function openFirstPaymentMode(mode) {
		if(mode == "add-money" || mode =="addMoney-bank")
	...
	function disablePaytmCash(){
		$('.fullWalletDeduct').addClass("f-hide");
		$('#remBal').hide();
		$("#paytmWalletCheckButton .blur-overlay").addClass("show");
	...
	function enablePaytmCash(){
		...
		$("#paytmWalletCheckButton .blur-overlay").removeClass("show");
		$(".no-walletTextUpdate").addClass("hide");
		$("#no-walletTextUpdate").siblings(".blur-overlay").hide();
	}
	...
	function toggleMerchantModes(show){
		...
		var submitParent = merchant.find("div.btn-submit").parent();
		var submitScParent = merchant.find("#scSubmit").parent().parent();

		submitParent.addClass("relative");

		var walletDetails = $("#wallet-details");
		var isWalletOnly = walletDetails.data("wallet-only");
		var addMoneyDetails = $('#addMoney-details');
		var addMoneyAvailable = addMoneyDetails.data("available");
		if(isWalletOnly && addMoneyAvailable){
			var merchant = $("#add-money-payment-modes");
			var control = merchant.find(".cards-control");
			var overlay = $("#add-money-payment-modes .card");
		...
		if(show == false){
			submitParent.append("<div class='disableSubmit'>");
			submitScParent.append("<div class='disableSubmit'>");
	...
	// Login strip shown after 10 sec interval irrespective of auth autologin response
	if(!isLoggedIn){
		setTimeout(function () {
			if($("#login-stitch").length > 0){
                $("#login-stitch").show();
			}
			if($("#login-wait").length > 0){
				$("#login-wait").hide();
			}
        },10000);
	}
...
//		}
	...
	if(isSubscription){
		...
		// hide checkbox
		// $(".walletCheckbox").hide();
		var paytmWalletEle = document.getElementById("paytm-cash-checkbox");
		var walletCheckboxEle = document.getElementById("walletCheckbox");
		if(paytmWalletEle){
			paytmWalletEle.disabled = true;
			var walletBootstrapCheckboxEle = paytmWalletEle.nextSibling;
			if(walletBootstrapCheckboxEle && walletBootstrapCheckboxEle.className.search("pcb") !== -1){
				walletBootstrapCheckboxEle.className += " disabled";
			}
		}
        if(subsMode == "PPBL"){
            var paytmPPBLEle = document.getElementById("paytm-paymentBank-checkbox");
            var PPBLCheckboxEle = document.getElementById("paytmPaymentBank");
            if(paytmPPBLEle) {
                paytmPPBLEle.disabled = true;
                var paytmPPBLBootstrapCheckboxEle = paytmPPBLEle.nextSibling;
                if(paytmPPBLBootstrapCheckboxEle && paytmPPBLBootstrapCheckboxEle.className.search("pcb") !== -1){
                    paytmPPBLBootstrapCheckboxEle.classList.add("disabled");
                }
			}
		}
		$(".payAmount .text-box").removeClass("ml20");

		// Set storeCardFlag to true and
		...
		if(ccSaveCardLabelEle){
			...
			ccSaveCardLabelEle.className += " relative";
		...
		if(dcSaveCardLabelEle){
			...
			dcSaveCardLabelEle.className += " relative";
...
//Fix Safari Bug (Login View Popup)
	...
	if(isSubscription && paymentBankEnabled && !paymentBankInactive && $("#paytm-paymentBank-checkbox").length>0) {
        if (!$("#paytm-paymentBank-checkbox")[0].checked) {
            $("#paytm-paymentBank-checkbox").click();
        }
	}
    ...
    function webSocketInit(){
        var orderDetails = $("#auth-js-details"),
            room,
            retryCount = 15,
            createOrderTimeout,createOrderTimeoutValue=360000,userInActiveTimeout,sessionTimeout,pnrSessionTimeout;
        ...
        socket.on('ConnectionEstablished', function(data) {
            ...
            socket.on("STATUS_RECEIVED",function(response){
				if(window.prnEnabled){
					if(response && response.data && response.data.status === "SUCCESS"){
						initPrnFlow(response.data);
                        ...
                        //
						// // Set prn timeout
						// pnrSessionTimeout = setTimeout(function () {
						// 	abandonTransaction();
						// },scanPayTimeout);
						return;
					}
    ...
    function getTransactionStatusForm(output){
		...
		var authDetail = $("#auth-js-details");
        ...
        form.method = "POST";
		var url = transactionStatusEndpoint;
        if(authDetail){
            url +=  "?MID=" + authDetail.data("mid") + "&ORDER_ID=" + authDetail.data("orderid");
        }
        form.action = url;
    ...
    if(!isLoggedIn && isQRCodeEnabled){
        ...
        webSocketInit();
        attachScanPayEvents(".pl5",".learnmore-image");
		togglePaymodeAccordion();

        // Scroll to top in scannpay
        setTimeout(function(){
            if(window.scrollTo){
                window.scrollTo(0,0);
            }
	...
	function togglePaymodeAccordion(){
		var ele = document.getElementById("scanpayToggleAccordion");
		if(ele){
			ele.addEventListener("click",function(e){
				var downArrowElm = document.getElementById("downArrowId");
				var rightArrowElm = document.getElementById("rightArrowId");
				var paymentOptionElm = document.getElementById("paymentOptionId");
				if(downArrowElm && rightArrowElm){
					if(downArrowElm.className.search("hide") !== -1){
						downArrowElm.className = downArrowElm.className.replace("hide","");
						rightArrowElm.className = "hide";
					}else{
						rightArrowElm.className = rightArrowElm.className.replace("hide","");
						downArrowElm.className = "hide";
					}
				}

				if(paymentOptionElm){
					paymentOptionElm.className = paymentOptionElm.className.search("hide") !== -1 ? "" : "hide";
				}
			 });
		}
	}
	...
	var prnPayload;
    function initPrnFlow(data){
        if(window.prnEnabled) {
            // Open prn screen when enabled
            prnPayload = data;
            var prnScanEle = document.getElementById("pnrScan");
            if (prnScanEle) {
                prnScanEle.className = prnScanEle.className.replace("hide", "");
            }
            document.body.className += " scroll-fix";
            attachScanPayEvents(".prn-learnmore",".prn-learnmore-image");

            // Attach prn validation event
            var prnSubmitEle = document.getElementById("prnSubmitId");
            var prnEle = document.getElementById("prnId");
            if (prnSubmitEle) {
                prnSubmitEle.addEventListener('click', prnValidate);
            }

            if(prnEle){
                prnEle.addEventListener('keyup',function(event){
                    // remove all character from prn ( only allow digits).
	...
	function prnValidate() {
        var prnEle = document.getElementById("prnId");
        var prnErrorEle = document.getElementById("prnValidateError");
        if(!prnEle){
            return;
        }
        ...
        if(prnEle.value === ""){
            if(prnErrorEle){
                prnErrorEle.innerText = "Payment Reference No. cannot be empty";
            }
            return;
        }

        if(prnEle.value.length <=7){
            if(prnErrorEle){
                prnErrorEle.innerText = "Payment Reference No. must be of 8 digit";
            }
            return;
        }

        var payload = {
            mid:$("#auth-js-details").data("mid"),
            orderId:authNewConfig.data('orderid'),
            prnCode:prnEle.value
        };
		var prnUrl = "/theia/validatePRN?MID=" + payload.mid + "&ORDER_ID=" + payload.orderId;
        $._post(prnUrl,payload,
            function(data, status) {
                if(data.status === "SUCCESS"){
                    ...
                    var form = getTransactionStatusForm(prnPayload);
                    document.body.appendChild(form);
                    form.submit();
                }else{
                    // Fail
                    if(data.retryAllowed){
                        // Retry Allowed
                        if(prnErrorEle){
                            prnErrorEle.innerText = "Enter the correct Payment Reference No.";
                        }
                    }else{
                        // Redirect to Abandon Transaction
                        abandonTransaction();
    ...
    (function() {
        //PG loaded
        pushGAData(false, 'payment_gateway_load');

        //Push whether user is LoggedIn
        if(isLoggedIn) {
            pushGAData(false, 'login_success');
        }

        //Case of autlogin
        var authConfig=JSON.parse($("#auth-config")[0].getAttribute('data-value'));
        if (authConfig.autoLogin != undefined && authConfig.autoLogin != "N") {
            pushGAData(false, 'automatic_login');
        }
    })();


    // Temp Fix - Paymode list is coming but content details is not shown
    setTimeout(function () {
        // $($(".cards-control a")[0].getAttribute("href")).addClass("active")
        var cards = $(".cards-control a").eq(0);
		if(cards && $(".cards-control .card.active").length == 0){
        	cards.addClass("active");
            var tabUrl = cards.attr("href");
            if(tabUrl){
...
if($){
    $._post = function(/* url, data, success, dataType */){
        ...
        options.type = 'POST';
        if(options.url && options.url.search("validatePRN") >= 0){
...
function setIrctcCharges(irData){
	...
	if( !((bm && tm && ct)) && !(typeof DEFAULTFEE_bm === 'undefined') && !(typeof DEFAULTFEE_cf === 'undefined') && !(typeof DEFAULTFEE_tm == "undefined")){
		bm = DEFAULTFEE_bm;
		cf= DEFAULTFEE_cf;
		tm = DEFAULTFEE_tm;
		ct = DEFAULTFEE_ct;
    ...
    //IRCTC ADD MONEY BOX TEXT UPDATE
    var addMoney=$("#irctcOnlyWallet");
    if(mode !== undefined && mode=="add-money" && addMoney.length>0){

        var bm = addMoney.data("bm");
        var ct = addMoney.data("ct");
        var tm = addMoney.data("tm");
        var cf = addMoney.data("cf");
    }


    var walletDetails = $("#wallet-details");
    var isWalletOnly = walletDetails.data("wallet-only");
    if(mode !== undefined && mode=="default" && isWalletOnly){
        var walletCharges=$("#irctcOnlyWallet");
        if(walletCharges){
            var bm = walletCharges.data("bm");
            var ct = walletCharges.data("ct");
            var tm = walletCharges.data("tm");
            var cf = walletCharges.data("cf");
		}
    }


    if($("body").find(".totalPcfAmt").length>0){
        $("body").find(".totalPcfAmt").text(tm);
        $("body .totalPcfAmt").each(function(){
            $(this).text(tm);
        });
    }
